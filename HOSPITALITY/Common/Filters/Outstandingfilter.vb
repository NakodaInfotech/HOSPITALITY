...
Public Class OutstandingFilter
    ...
    Private Sub cmdshow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdshow.Click
        Try
            ...
            If CMBGROUP.Text.Trim <> "" Then OBJOUTSTAND.selfor_ss = OBJOUTSTAND.selfor_ss & " and {@PARTYGROUP}='" & CMBGROUP.Text.Trim & "'"


            If CMBPARTYNAME.Text <> "" Then
                OBJOUTSTAND.selfor_ss = OBJOUTSTAND.selfor_ss & " and {@NAME}='" & CMBPARTYNAME.Text.Trim & "'"
            Else
                ...
                For i As Integer = 0 To gridbill.RowCount - 1
                    ...
                    If Convert.ToBoolean(dtrow("CHK")) = True Then
                        If NAMECLAUSE = "" Then
                            If RBGROUP.Checked = True Then NAMECLAUSE = " AND ({@PARTYGROUP} = '" & dtrow("NAME") & "'" Else NAMECLAUSE = " AND ({@NAME} = '" & dtrow("NAME") & "'"
                        Else
                            If RBGROUP.Checked = True Then NAMECLAUSE = NAMECLAUSE & " OR {@PARTYGROUP} = '" & dtrow("NAME") & "'" Else NAMECLAUSE = NAMECLAUSE & " OR {@NAME} = '" & dtrow("NAME") & "'"
                        End If
                    End If
                Next
    ...
    Sub FILLGRID()
        Try
            ...
            If RBACCOUNT.Checked = True Then
                dt = objclsCMST.search(" CAST (0 AS BIT) AS CHK,LEDGERS.Acc_cmpname AS NAME, GROUPMASTER.group_secondary AS UNDER, ISNULL(CITYMASTER.city_name, '') AS CITY  ", " ", " LEDGERS INNER JOIN GROUPMASTER ON LEDGERS.Acc_groupid = GROUPMASTER.group_id LEFT OUTER JOIN CITYMASTER ON LEDGERS.Acc_cityid = CITYMASTER.city_id ", WHERECLAUSE & " AND (LEDGERS.ACC_YEARID = '" & YearId & "') ORDER BY LEDGERS.Acc_cmpname")
            Else
                dt = objclsCMST.search(" CAST (0 AS BIT) AS CHK, group_name AS NAME, group_under AS UNDER, group_secondary AS CITY ", " ", " GROUPMASTER ", " AND (GROUPMASTER.GROUP_YEARID = '" & YearId & "') ORDER BY group_name")
            End If
