...
Public Class RefWiseSaleDetails

    Public FRMSTRING As String = ""
    Public WHERESTRING As String = ""

    Private Sub RefWiseSaleDetails_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
        ...
        Try
            ...
            If FRMSTRING = "SALEBALANCE" Then
                If ClientName = "PARAMOUNT" Then
                    dt = objclsCMST.search(" BILL, BOOKINGNO, DATE, REFNO, GUEST_NAME AS GUESTNAME, SALELEDGER AS NAME, HOTELNAME, ARRIVAL, DEPARTURE, BOOKEDBY, HOLIDAYPACKAGEMASTER.BOOKING_SUBTOTAL AS SUBTOTAL, HOLIDAYPACKAGEMASTER.BOOKING_TAX AS TAX, SALEAMT AS BILLAMT, SALEBAL AS DEBITBAL , 0 AS CREDITBAL, BOOKTYPE, TYPE ", "", " GUESTMASTER INNER JOIN HOLIDAYPACKAGEMASTER_GUESTDETAILS ON GUESTMASTER.GUEST_ID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_GUESTID AND GUESTMASTER.GUEST_CMPID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_CMPID AND GUESTMASTER.GUEST_LOCATIONID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_LOCATIONID AND GUESTMASTER.GUEST_YEARID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_YEARID INNER JOIN RESERVATION_SALEREPORT ON HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_NO = RESERVATION_SALEREPORT.BILL AND HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_CMPID = RESERVATION_SALEREPORT.CMPID AND HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_LOCATIONID = RESERVATION_SALEREPORT.LOCATIONID AND HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_YEARID = RESERVATION_SALEREPORT.YEARID INNER JOIN HOLIDAYPACKAGEMASTER ON RESERVATION_SALEREPORT.BILL = HOLIDAYPACKAGEMASTER.BOOKING_NO AND RESERVATION_SALEREPORT.CMPID = HOLIDAYPACKAGEMASTER.BOOKING_CMPID AND RESERVATION_SALEREPORT.LOCATIONID = HOLIDAYPACKAGEMASTER.BOOKING_LOCATIONID AND RESERVATION_SALEREPORT.YEARID = HOLIDAYPACKAGEMASTER.BOOKING_YEARID  ", WHERECLAUSE & " AND CANCELLED = 'FALSE' AND SALEAMT > 0 AND SALEBAL > 0 AND CMPID =" & CmpId & " AND LOCATIONID = " & Locationid & " AND YEARID = " & YearId & " ORDER BY RESERVATION_SALEREPORT.DATE,BILL")
                Else
                    dt = objclsCMST.search(" BILL, BOOKINGNO, DATE, REFNO, GUESTNAME, HOTELNAME, ARRIVAL, DEPARTURE, BOOKEDBY, SALEAMT AS BILLAMT, SALEBAL AS DEBITBAL , 0 AS CREDITBAL, BOOKTYPE, TYPE ", "", " RESERVATION_SALEREPORT ", WHERECLAUSE & " AND CANCELLED = 'FALSE' AND SALEAMT > 0 AND SALEBAL > 0 AND CMPID =" & CmpId & " AND LOCATIONID = " & Locationid & " AND YEARID = " & YearId & " ORDER BY RESERVATION_SALEREPORT.DATE,BILL")
                End If
            ...
            ElseIf FRMSTRING = "ALLBALANCE" Then
                If ClientName = "PARAMOUNT" Then
                    dt = objclsCMST.search(" * ", "", "( SELECT BOOKINGNO AS BILL,BILLINITIALS AS BOOKINGNO, BOOKINGDATE AS DATE, REFNO, GUESTMASTER.GUEST_NAME AS GUESTNAME, GUESTNAME AS NAME, HOTELNAME, ARRIVAL, DEPARTURE, BOOKEDBY, NETTAMT AS SUBTOTAL , TAX, GTOTAL AS BILLAMT, 0 AS DEBITBAL, CREDITBALANCE AS CREDITBAL, BOOKTYPE, TYPE  FROM RESERVATIONDETAILS INNER JOIN HOLIDAYPACKAGEMASTER_GUESTDETAILS ON RESERVATIONDETAILS.BOOKINGNO = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_NO AND RESERVATIONDETAILS.CMPID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_CMPID AND RESERVATIONDETAILS.LOCATIONID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_LOCATIONID AND RESERVATIONDETAILS.YEARID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_YEARID INNER JOIN GUESTMASTER ON HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_GUESTID = GUESTMASTER.GUEST_ID AND HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_CMPID = GUESTMASTER.GUEST_CMPID AND HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_LOCATIONID = GUESTMASTER.GUEST_LOCATIONID AND HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_YEARID = GUESTMASTER.GUEST_YEARID  WHERE 1 = 1 " & WHERECLAUSE & " AND CANCELLED = 0 AND CREDIT > 0 AND CREDITBALANCE > 0 AND CMPID =" & CmpId & " AND LOCATIONID = " & Locationid & " AND YEARID = " & YearId & " UNION ALL SELECT BILL, BOOKINGNO, DATE, REFNO, GUEST_NAME AS GUESTNAME, SALELEDGER AS NAME, HOTELNAME, ARRIVAL, DEPARTURE, BOOKEDBY, HOLIDAYPACKAGEMASTER.BOOKING_SUBTOTAL AS SUBTOTAL, HOLIDAYPACKAGEMASTER.BOOKING_TAX AS TAX, SALEAMT AS BILLAMT, SALEBAL AS DEBITBAL , 0 AS CREDITBAL, BOOKTYPE, TYPE  FROM GUESTMASTER INNER JOIN HOLIDAYPACKAGEMASTER_GUESTDETAILS ON GUESTMASTER.GUEST_ID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_GUESTID AND GUESTMASTER.GUEST_CMPID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_CMPID AND GUESTMASTER.GUEST_LOCATIONID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_LOCATIONID AND GUESTMASTER.GUEST_YEARID = HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_YEARID INNER JOIN RESERVATION_SALEREPORT ON HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_NO = RESERVATION_SALEREPORT.BILL AND HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_CMPID = RESERVATION_SALEREPORT.CMPID AND HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_LOCATIONID = RESERVATION_SALEREPORT.LOCATIONID AND HOLIDAYPACKAGEMASTER_GUESTDETAILS.BOOKING_YEARID = RESERVATION_SALEREPORT.YEARID INNER JOIN HOLIDAYPACKAGEMASTER ON RESERVATION_SALEREPORT.BILL = HOLIDAYPACKAGEMASTER.BOOKING_NO AND RESERVATION_SALEREPORT.CMPID = HOLIDAYPACKAGEMASTER.BOOKING_CMPID AND RESERVATION_SALEREPORT.LOCATIONID = HOLIDAYPACKAGEMASTER.BOOKING_LOCATIONID AND RESERVATION_SALEREPORT.YEARID = HOLIDAYPACKAGEMASTER.BOOKING_YEARID  WHERE 1 = 1 " & WHERECLAUSE & " AND CANCELLED = 'FALSE' AND SALEAMT > 0 AND SALEBAL > 0 AND CMPID =" & CmpId & " AND LOCATIONID = " & Locationid & " AND YEARID = " & YearId & ") AS T", " ORDER BY T.DATE,T.BOOKINGNO ")
                Else
                    dt = objclsCMST.search(" * ", "", "( SELECT BOOKINGNO AS BILL,BILLINITIALS AS BOOKINGNO, BOOKINGDATE AS DATE, REFNO, GUESTNAME, HOTELNAME, ARRIVAL, DEPARTURE, BOOKEDBY, GTOTAL AS BILLAMT, 0 AS DEBITBAL, CREDITBALANCE AS CREDITBAL, BOOKTYPE, TYPE FROM RESERVATIONDETAILS WHERE 1 = 1 " & WHERECLAUSE & " AND CANCELLED = 0 AND CREDIT > 0 AND CREDITBALANCE > 0 AND CMPID =" & CmpId & " AND LOCATIONID = " & Locationid & " AND YEARID = " & YearId & " UNION ALL SELECT BILL, BOOKINGNO, DATE, REFNO, GUESTNAME, HOTELNAME, ARRIVAL, DEPARTURE, BOOKEDBY, SALEAMT AS BILLAMT, SALEBAL AS DEBITBAL , 0 AS CREDITBAL,  BOOKTYPE, TYPE FROM RESERVATION_SALEREPORT WHERE 1 = 1 " & WHERECLAUSE & " AND CANCELLED = 'FALSE' AND SALEAMT > 0 AND SALEBAL > 0 AND CMPID =" & CmpId & " AND LOCATIONID = " & Locationid & " AND YEARID = " & YearId & ") AS T", " ORDER BY T.DATE,T.BOOKINGNO ")
                End If
            End If
    ...
    Private Sub RefWiseSaleDetails_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If FRMSTRING = "SALEBALANCE" Then
    ...
    Private Sub cmdshowdetails_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdshowdetails.Click
        Try
            If chkdate.CheckState = CheckState.Checked Then WHERESTRING = WHERESTRING & " AND DATE >= '" & Format(dtfrom.Value.Date, "MM/dd/yyyy") & "' AND DATE <= '" & Format(dtto.Value.Date, "MM/dd/yyyy") & "' "
            fillgrid(WHERESTRING)
        Catch ex As Exception
            Throw ex
        End Try
