...
Public Class HotelWiseSaleReport

    Public SALETYPE As String
    Public TEMPCITYNAME As String

    Private Sub HotelWiseSaleReport_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Sub fillgrid(ByVal WHERECLAUSE As String)
        Try
            ...
            dt = objclsCMST.search(" TYPE, CITYNAME, HOTELNAME, SUM(PURAMT) AS PURAMT, SUM(SALEAMT) AS SALEAMT, SUM(GROSSAMT) AS GROSSAMT, (CASE WHEN SUM(GROSSAMT) > 0 AND SUM(SALEAMT) > 0 THEN ROUND((SUM(GROSSAMT)/SUM(SALEAMT))* 100,2) ELSE 0 END) AS GROSSPER, SUM(ROOMS) AS ROOMS, SUM(NIGHTS) AS NIGHTS, SUM(TOTALNIGHTS) AS TOTALNIGHTS ", "", " CITYWISESALE ", WHERECLAUSE & " AND CMPID =" & CmpId & " AND LOCATIONID = " & Locationid & " AND YEARID = " & YearId & " GROUP BY TYPE, CITYNAME, HOTELNAME ORDER BY TYPE, CITYNAME, HOTELNAME ")
    ...
    Sub SHOWFORM()
        Try
            Dim OBJ As New ReservationDetails
            OBJ.STRSEARCH = " AND TYPE <> 'OPENING'  AND YEARID = " & YearId & " AND TYPE = '" & SALETYPE & "' AND HOTELNAME = '" & gridbill.GetFocusedRowCellValue("HOTELNAME") & "'"
            If chkdate.CheckState = CheckState.Checked Then OBJ.STRSEARCH = OBJ.STRSEARCH & " AND DATE >= '" & Format(dtfrom.Value.Date, "MM/dd/yyyy") & "' AND DATE<= '" & Format(dtto.Value.Date, "MM/dd/yyyy") & "'"
    ...
    Private Sub HotelWiseSaleReport_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            fillgrid("")
        Catch ex As Exception
            Throw ex
