...
Public Class InterestCalc_Summary
    ...
    Sub fillgrid()
        Try
            ...
            Dim GROUP As String = ""
            If cmbgroup.Text <> "" Then GROUP = GROUP & " AND GROUPMASTER.GROUP_SECONDARY ='" & cmbgroup.Text.Trim & "'"

            'THIS CODE DOES NOT RETRIEVES OPENING ACCOUNTS
            'Dim NAMEDT As DataTable = OBJCMN.search(" 1 AS SORTNO, NAME,GROUPMASTER.group_secondary AS GROUPSECONDARY, GROUPMASTER.GROUP_NAME AS GROUPNAME, SUM(DEBIT) AS DEBIT, SUM(CREDIT) AS CREDIT, (CASE WHEN (SUM(DEBIT) - SUM(CREDIT)) > 0 THEN (SUM(DEBIT) - SUM(CREDIT)) ELSE (SUM(CREDIT) - SUM(DEBIT)) END) AS NETTBALANCE, " & Val(TXTPERCENT.Text.Trim) & " AS INTPER, 0 AS TOPAY, 0 AS TOREC, 0 AS INTDR, 0 AS INTCR, LEDGERS.ACC_PANNO AS PANNO, ACCOUNTSMASTER_TDS.ACC_TDSPER AS TDSPER, ACCOUNTSMASTER_TDS.ACC_TDSFORM AS TDSFORM ", "", " (SELECT ROW_NUMBER() OVER (ORDER BY DUEDATE)AS ROWNO, SRNO, REGTYPE, BILLINITIALS, TYPE, NAME, DATE, APPDATE, DUEDATE,TOTALBALES, DEBIT, CREDIT,  CMPID, LOCATIONID, YEARID FROM INTERESTVIEW WHERE YEARID =  " & YearId & " " & WHERE & "   ) AS NEWT INNER JOIN LEDGERS ON NEWT.YEARID = LEDGERS.Acc_yearid AND NEWT.NAME = LEDGERS.Acc_CMPname INNER JOIN GROUPMASTER ON LEDGERS.Acc_groupid = GROUPMASTER.group_id LEFT OUTER JOIN ACCOUNTSMASTER_TDS ON LEDGERS.ACC_ID = ACCOUNTSMASTER_TDS.ACC_ID ", " AND YEARID = " & YearId & GROUP & " GROUP BY NAME,GROUPMASTER.group_secondary, GROUPMASTER.GROUP_NAME, LEDGERS.ACC_PANNO, ACCOUNTSMASTER_TDS.ACC_TDSFORM, ACCOUNTSMASTER_TDS.ACC_TDSPER ORDER BY NEWT.NAME ")
            Dim NAMEDT As DataTable = OBJCMN.search(" 1 AS SORTNO, NAME,GROUPMASTER.group_secondary AS GROUPSECONDARY, GROUPMASTER.GROUP_NAME AS GROUPNAME, SUM(DEBIT) AS DEBIT, SUM(CREDIT) AS CREDIT, (CASE WHEN (SUM(DEBIT) - SUM(CREDIT)) > 0 THEN (SUM(DEBIT) - SUM(CREDIT)) ELSE (SUM(CREDIT) - SUM(DEBIT)) END) AS NETTBALANCE, " & Val(TXTPERCENT.Text.Trim) & " AS INTPER, 0 AS TOPAY, 0 AS TOREC, 0 AS INTDR, 0 AS INTCR, LEDGERS.ACC_PANNO AS PANNO, ACCOUNTSMASTER_TDS.ACC_TDSRATE AS TDSPER, 0 AS TDSAMT, 0 AS NETTINT, 'TDS' AS TDSFORM ", "", " (SELECT ROW_NUMBER() OVER (ORDER BY DUEDATE)AS ROWNO, SRNO, REGTYPE, BILLINITIALS, TYPE, NAME, DATE,  DUEDATE, DEBIT, CREDIT,  CMPID, LOCATIONID, YEARID FROM INTERESTVIEW WHERE YEARID =  " & YearId & " " & WHERE & " UNION ALL SELECT ROW_NUMBER() OVER (ORDER BY DUEDATE)AS ROWNO, SRNO, REGTYPE, BILLINITIALS, TYPE, NAME, DATE,  DUEDATE, DEBIT, CREDIT,  CMPID, LOCATIONID, YEARID FROM INTERESTVIEW WHERE 1=1 " & OPWHERE & ") AS NEWT INNER JOIN LEDGERS ON NEWT.YEARID = LEDGERS.Acc_yearid AND NEWT.NAME = LEDGERS.Acc_CMPname INNER JOIN GROUPMASTER ON LEDGERS.Acc_groupid = GROUPMASTER.group_id LEFT OUTER JOIN ACCOUNTSMASTER_TDS ON LEDGERS.ACC_ID = ACCOUNTSMASTER_TDS.ACC_ID ", " AND YEARID = " & YearId & GROUP & " GROUP BY NAME,GROUPMASTER.group_secondary, GROUPMASTER.GROUP_NAME, LEDGERS.ACC_PANNO, ACCOUNTSMASTER_TDS.ACC_TDSRATE ORDER BY NEWT.NAME ")
            ...
            If NAMEDT.Rows.Count > 0 Then
                ...
                For Each NAMEROW As DataRow In NAMEDT.Rows
                    ...
                    '    Dim DTPARTYINT As DataTable = OBJCMN.search(" ACC_INTPER AS INTPER ", "", " LEDGERS ", " AND ACC_CMPNAME = '" & NAMEROW("NAME") & "' AND ACC_YEARID = " & YearId)
                    '    If Val(DTPARTYINT.Rows(0).Item("INTPER")) > 0 Then NAMEROW("INTPER") = Val(DTPARTYINT.Rows(0).Item("INTPER"))
                    'End If

                    Dim dt As DataTable = OBJCMN.search("1 AS SORTNO, SRNO, REGTYPE, BILLINITIALS, TYPE, NAME, DATE,  DUEDATE, DEBIT, CREDIT, ISNULL( DATEDIFF(DAY,(SELECT DUEDATE FROM (SELECT     ROW_NUMBER() OVER ( ORDER BY DUEDATE)AS ROWNO, DUEDATE FROM INTERESTVIEW WHERE (NAME = '" & NAMEROW("NAME") & "' AND YEARID = " & YearId & WHERE & ")) AS T WHERE T.ROWNO = NEWT.ROWNO -1 ), NEWT.[DUEDATE]),0) AS [DAYS], 0 AS NETTBALANCE, 0 AS TOPAY, 0 AS TOREC  ", "", " (SELECT     ROW_NUMBER() OVER (ORDER BY DUEDATE)AS ROWNO, SRNO, REGTYPE, BILLINITIALS, TYPE, NAME, DATE,  DUEDATE, DEBIT, CREDIT,  CMPID, LOCATIONID, YEARID FROM INTERESTVIEW WHERE (NAME = '" & NAMEROW("NAME") & "' AND YEARID = " & YearId & WHERE & ")) AS NEWT ", WHERE & " ORDER BY NEWT.NAME, NEWT.DUEDATE  ")
                    ...
                    If Val(NAMEROW("TOPAY")) > 0 Then NAMEROW("NETTINT") = Val(NAMEROW("TOPAY")) - Val(NAMEROW("TDSAMT")) Else NAMEROW("NETTINT") = Val(NAMEROW("TOREC")) - Val(NAMEROW("TDSAMT"))
    ...
    Sub TOTAL(ByVal PERCENTCAL As Double, ByVal LEDGERNAME As String)
        Try
            ...
            For i = 0 To gridregister.RowCount - 1
                ...
                'Dim DTPARTYINT As DataTable = OBJCMN.search("ACC_INTPER AS INTPER ", "", " LEDGERS ", " AND ACC_CMPNAME = '" & LEDGERNAME & "' AND ACC_YEARID = " & YearId)
                'If CHKALL.CheckState = CheckState.Checked Then  Else PARTYINTPER = Val(DTPARTYINT.Rows(0).Item("INTPER"))
                PARTYINTPER = PERCENTCAL
                ...
                If dtrow("BILLINITIALS") = "CLOSING" Then
                    ...
                    If dtrow("DUEDATE") = TEMPDTROW("DUEDATE") And TEMPDTROW("BILLINITIALS") <> "OPENING" Then
                        ...
                        If TEMPDTROW("NETTBALANCE") > 0 Then
                            TEMPDTROW("TOREC") = (((Val(PERCENTCAL) * Val(TEMPDTROW1("NETTBALANCE"))) / 100) / Val(TXTDAYS.Text.Trim) * Val(TEMPDTROW("DAYS")))
                        Else
                            TEMPDTROW("TOPAY") = (((Val(PERCENTCAL) * (Val(TEMPDTROW1("NETTBALANCE")) * (-1))) / 100) / Val(TXTDAYS.Text.Trim) * Val(TEMPDTROW("DAYS")))
                        End If
                        dtrow("DAYS") = 0
                    End If
                End If
                ...
                If Val(dtrow("DAYS")) > 0 Then
                    If Val(RUNNINGBAL) > 0 Then
                        dtrow("TOREC") = (((Val(PERCENTCAL) * RUNNINGBAL) / 100) / Val(TXTDAYS.Text.Trim) * Val(dtrow("DAYS")))
                    Else
                        dtrow("TOPAY") = (((Val(PERCENTCAL) * (RUNNINGBAL * (-1))) / 100) / Val(TXTDAYS.Text.Trim) * Val(dtrow("DAYS")))
                    End If
                End If
                RUNNINGBAL = dtrow("NETTBALANCE")
            Next
    ...
    Private Sub TOOLPRINT_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TOOLPRINT.Click
        Try
            ...
            For I As Integer = 0 To GRIDNAMEREGISTER.RowCount - 1
                ...
                DTTDS = OBJCMN.search("ISNULL(ACC_TDSRATE,0) AS TDSPER, 'TDS' AS TDSFORM ", "", " LEDGERS INNER JOIN ACCOUNTSMASTER_TDS ON LEDGERS.Acc_id = ACCOUNTSMASTER_TDS.ACC_ID  ", " AND ACC_CMPNAME = '" & DTROW("NAME") & "' AND LEDGERS.ACC_YEARID = " & YearId)
