...
Public Class PaymentMaster
    ...
    Sub GETBALANCE()
        Try
            ...
            Dim DT As DataTable = OBJCMN.search("(CASE WHEN DR > 0 THEN 'Dr'  ELSE 'Cr' END) AS SALEBAL, isnull(ACC_CRLIMIT,0) AS CRLIMIT, (CASE WHEN DR > 0 THEN DR ELSE CR END) AS BALANCE ", "", "  TRIALBALANCE INNER JOIN LEDGERS ON TRIALBALANCE.LEDGERID = LEDGERS.Acc_ID ", " AND NAME = '" & cmbaccname.Text.Trim & "' AND YEARID = " & YearId)
            ...
            DT = OBJCMN.search("(CASE WHEN DR > 0 THEN 'Dr'  ELSE 'Cr' END) AS SALEBAL, isnull(ACC_CRLIMIT,0) AS CRLIMIT, (CASE WHEN DR > 0 THEN DR ELSE CR END) AS BALANCE ", "", "  TRIALBALANCE INNER JOIN LEDGERS ON TRIALBALANCE.LEDGERID = LEDGERS.Acc_ID ", " AND NAME = '" & cmbname.Text.Trim & "' AND YEARID = " & YearId)
            If DT.Rows.Count > 0 Then
    ...
    Sub clear()
        ...
        txtinwords.Clear()
        txtsrno.Clear()
        If ClientName <> "KHANNA" Then cmbpaytype.SelectedIndex = 0 Else cmbpaytype.SelectedIndex = 2
        txtamt.Clear()
        cmbbillno.Text = ""
        txtnarr.Clear()
    ...
    Private Sub txtinwords_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtinwords.KeyPress
        enterkeypress(e, Me)
    End Sub
        ...
        Try
            ...
            If ClientName = "CLASSIC" Then
                If UserName <> "Admin" And edit = False Then
                    If ACCDATE.Value.Date < Now.Date Then
                        EP.SetError(ACCDATE, "Back Date Entry Not Allowed")
                        BLN = False
                    End If
                End If
            End If
            ...
            If ClientName = "CLASSIC" Then
                If UserName <> "Admin" And edit = True Then
                    If ACCDATE.Value.Date < DDATE.Date Then
                        EP.SetError(ACCDATE, "Back Date Entry Not Allowed")
                        BLN = False
                    End If
                End If
    ...
    Private Sub PaymentMaster_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        ElseIf e.Alt = True And e.KeyCode = Windows.Forms.Keys.F1 Then
            Dim OBJINVDTLS As New PaymentDetails
    ...
    Private Sub PaymentMaster_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            getmaxno_PAYMENTmaster()
            fillledger(cmbname, edit, " and (groupmaster.group_SECONDARY = 'Sundry Creditors' or groupmaster.group_SECONDARY = 'Indirect Expenses' or groupmaster.group_SECONDARY = 'Direct Expenses') and acc_cmpid = " & CmpId & " and acc_LOCATIONid = " & Locationid & " and acc_YEARid = " & YearId)
            ...
            FILLTOURNAME(CMBGROUPDEPARTURE, edit, "")
            ...
            If edit = True Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    For Each dr As DataRow In dt.Rows
                        ...
                        CMBGROUPDEPARTURE.Text = Convert.ToString(dr("TOURNAME"))
    ...
    Private Sub cmbname_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbname.Validated
        Try
            ...
            If cmbname.Text.Trim <> "" And ClientName = "CLASSIC" Then GETBALANCE()
    ...
    Private Sub cmdsave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdsave.Click
        Try
            ...
            'SEND SMS
            If ClientName = "TRINITY" Then
                If MsgBox("Send SMS To Client?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then SMSCODE()
            End If
    ...
    Private Sub cmbaccname_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmbaccname.KeyPress
        enterkeypress(e, Me)
    End Sub
    ...
    Sub SETGRIDINVOICE(ByVal DT As DataTable)
        Try
            ...
            GRIDBILL.Columns(10).HeaderText = "Party Bill"
    ...
    Private Sub gridpayment_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridpayment.CellClick
        Try
            If ClientName = "AIRTRINITY" And UserName <> "ADMIN" Then Exit Sub
    ...
    Private Sub gridPAYMENT_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridpayment.CellDoubleClick
        Try
            ...
            If e.RowIndex >= 0 And gridpayment.Item(gridsrno.Index, e.RowIndex).Value <> Nothing Then
                ...
                If Val(gridpayment.Item(GAMTREC.Index, e.RowIndex).Value) > 0 Or Val(gridpayment.Item(GEXTRAAMT.Index, e.RowIndex).Value) > 0 Or Val(gridpayment.Item(GRETURN.Index, e.RowIndex).Value) > 0 Then
                    MsgBox("Payment Locked, Delete Rec First", MsgBoxStyle.Critical)
                    Exit Sub
                End If
                ...
                If cmbbillno.Text.Trim <> "" Then
                    ...
                    Dim OBJCMN As New ClsCommon
                    Dim DT As DataTable = OBJCMN.search(" T.BALANCE AS BALAMT", "", " (SELECT BILL_INITIALS AS BILLINITIALS, BILL_BALANCE AS BALANCE, BILL_CMPID AS CMPID , BILL_LOCATIONID AS LOCATIONID , BILL_YEARID AS YEARID FROM OPENINGBILL  UNION ALL SELECT HOTELBOOKINGMASTER.BOOKING_PURBILLINITIALS AS BILLINITIALS, HOTELBOOKINGMASTER.BOOKING_PURBALANCE AS BALANCE, BOOKING_CMPID AS CMPID , BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM HOTELBOOKINGMASTER UNION ALL SELECT RAILBOOKINGMASTER.BOOKING_PURBILLINITIALS AS BILLINITIALS, RAILBOOKINGMASTER.BOOKING_PURBALANCE AS BALANCE, BOOKING_CMPID AS CMPID , BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM RAILBOOKINGMASTER UNION ALL SELECT BOOKING_PURBILLINITIALS AS BILLINITIALS, BOOKING_PURBALANCE AS BALANCE, BOOKING_CMPID AS CMPID , BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM MISCPURMASTER UNION ALL SELECT BOOKING_PURBILLINITIALS AS BILLINITIALS, BOOKING_BALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID, BOOKING_YEARID AS YEARID FROM CARBOOKINGMASTER_PURCHASEDETAILS  UNION ALL SELECT AIRLINEBOOKINGMASTER.BOOKING_PURBILLINITIALS AS BILLINITIALS, AIRLINEBOOKINGMASTER.BOOKING_PURBALANCE AS BALANCE, BOOKING_CMPID AS CMPID , BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM AIRLINEBOOKINGMASTER UNION ALL SELECT BOOKING_PURBILLINITIALS AS BILLINITIALS, BOOKING_BALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID, BOOKING_YEARID AS YEARID FROM HOLIDAYPACKAGEMASTER_PURCHASEDETAILS UNION ALL SELECT     BOOKING_PURBILLINITIALS AS BILLINITIALS, BOOKING_BALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID, BOOKING_YEARID AS YEARID FROM INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS UNION ALL	SELECT JOURNALMASTER.JOURNAL_INITIALS AS BILLINITIALS, (SUM(JOURNAL_DEBIT)-(JOURNAL_AMT + JOURNAL_TDS)) AS BALANCE, JOURNAL_CMPID AS CMPID, JOURNAL_LOCATIONID AS LOCATIONID , JOURNAL_YEARID AS YEARID FROM JOURNALMASTER GROUP BY journal_initials,journal_amt, journal_tds, JOURNAL_CMPID, JOURNAL_LOCATIONID, JOURNAL_YEARID UNION ALL	SELECT NONPURCHASE.NP_INITIALS AS BILLINITIALS, NP_BALANCE AS BALANCE, NP_CMPID AS CMPID, NP_LOCATIONID AS LOCATIONID , NP_YEARID AS YEARID  FROM NONPURCHASE UNION ALL SELECT SAL_INITIALS AS BILLINITIALS, SAL_BALANCE AS BALANCE, SAL_CMPID AS CMPID , SAL_LOCATIONID AS LOCATIONID , SAL_YEARID AS YEARID FROM SALARYMASTER UNION ALL SELECT CAST(RECEIPTMASTER_DESC.RECEIPT_GRIDREMARKS AS VARCHAR(100)) AS BILLINITIALS, RECEIPTMASTER_DESC.RECEIPT_BALANCE AS BALANCE, RECEIPT_CMPID AS CMPID , 0 AS LOCATIONID , RECEIPT_YEARID AS YEARID FROM RECEIPTMASTER_DESC WHERE RECEIPT_paytype = 'New Ref' AND ISNULL(RECEIPT_GRIDREMARKS,'')  <> '') AS T", " AND T.BILLINITIALS = '" & cmbbillno.Text.Trim & "' AND T.CMPID = " & CmpId & " AND T.LOCATIONID = " & Locationid & " AND T.YEARID = " & YearId)
                    If DT.Rows.Count > 0 Then
    ...
    Private Sub gridPAYMENT_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles gridpayment.KeyDown
        If e.KeyCode = Keys.Delete Then
            ...
            If Val(gridpayment.Item(GAMTREC.Index, gridpayment.CurrentRow.Index).Value) > 0 Or Val(gridpayment.Item(GEXTRAAMT.Index, gridpayment.CurrentRow.Index).Value) > 0 Or Val(gridpayment.Item(GRETURN.Index, gridpayment.CurrentRow.Index).Value) > 0 Then
                MsgBox("Payment Locked, Delete Rec First", MsgBoxStyle.Critical)
                Exit Sub
            End If
...
LINE1:
    ...
    Private Sub cmbpaytype_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmbpaytype.KeyPress
        enterkeypress(e, Me)
    End Sub
    ...
    Private Sub cmbaccname_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbaccname.Validated
        If edit = False Then
            Dim OBJCMN As New ClsCommon
            Dim DT As DataTable = OBJCMN.search(" ISNULL(MAX(CAST(PAYMENT_CHQNO AS VARCHAR)),0), GROUP_SECONDARY", "", " PAYMENTMASTER INNER JOIN LEDGERS ON ACC_ID = PAYMENT_ACCID INNER JOIN GROUPMASTER ON GROUP_ID = ACC_GROUPID INNER JOIN REGISTERMASTER ON REGISTER_ID = PAYMENT_REGISTERID ", " AND ACC_CMPNAME = '" & cmbaccname.Text.Trim & "' AND REGISTER_NAME = '" & cmbregister.Text.Trim & "' AND PAYMENT_YEARID = " & YearId & " AND (GROUP_SECONDARY = 'BANK A/C' OR GROUP_SECONDARY = 'BANK OD A/C') GROUP BY GROUP_SECONDARY")
            If DT.Rows.Count > 0 Then
                On Error Resume Next
                TXTCHQNO.Text = DT.Rows(0).Item(0) + 1
            End If
        End If
        'If cmbaccname.Text.Trim <> "" And ClientName = "CLASSIC" Then GETBALANCE()
    End Sub
    ...
    Private Sub txtbillno_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtbillno.Validating
        Try
            ...
            If txtbillno.Text.Trim <> "" Then
                ...
                'CHECKING IN HOTELBOOKING
                DT = OBJCMN.search("HOTELBOOKINGMASTER.BOOKING_PURBILLINITIALS AS BILLINITIALS, HOTELBOOKINGMASTER.BOOKING_DATE AS BILLDATE, HOTELBOOKINGMASTER.BOOKING_PURBALANCE AS BALAMT, HOTELBOOKINGMASTER.BOOKING_PURGRANDTOTAL AS BILLAMT, 'PURCHASE' AS BILLTYPE, HOTELBOOKINGMASTER.BOOKING_NO AS BILLNO,  REGISTERMASTER.register_name AS REGTYPE, LEDGERS.ACC_CMPNAME AS NAME", "", " HOTELBOOKINGMASTER INNER JOIN REGISTERMASTER ON HOTELBOOKINGMASTER.BOOKING_PURCHASEREGISTERID = REGISTERMASTER.register_id AND HOTELBOOKINGMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND HOTELBOOKINGMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND HOTELBOOKINGMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON HOTELBOOKINGMASTER.BOOKING_PURLEDGERID = LEDGERS.Acc_id AND HOTELBOOKINGMASTER.BOOKING_CMPID = LEDGERS.Acc_cmpid AND HOTELBOOKINGMASTER.BOOKING_LOCATIONID = LEDGERS.Acc_locationid AND HOTELBOOKINGMASTER.BOOKING_YEARID = LEDGERS.Acc_yearid ", " AND ( HOTELBOOKINGMASTER.BOOKING_PURBILLINITIALS = '" & txtbillno.Text.Trim & "') AND HOTELBOOKINGMASTER.BOOKING_PURBALANCE > 0  AND (HOTELBOOKINGMASTER.BOOKING_cmpid = " & CmpId & ") AND (HOTELBOOKINGMASTER.BOOKING_locationid = " & Locationid & ") AND (HOTELBOOKINGMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                'CHECKING IN MISCPURCHASE
                DT = OBJCMN.search("BOOKING_PURBILLINITIALS AS BILLINITIALS, BOOKING_DATE AS BILLDATE, BOOKING_PURBALANCE AS BALAMT, BOOKING_PURGRANDTOTAL AS BILLAMT, 'MISCPURCHASE' AS BILLTYPE, BOOKING_NO AS BILLNO,  REGISTERMASTER.register_name AS REGTYPE, LEDGERS.ACC_CMPNAME AS NAME", "", " MISCPURMASTER INNER JOIN REGISTERMASTER ON MISCPURMASTER.BOOKING_PURCHASEREGISTERID = REGISTERMASTER.register_id AND MISCPURMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND MISCPURMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND MISCPURMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON MISCPURMASTER.BOOKING_PURLEDGERID = LEDGERS.Acc_id AND MISCPURMASTER.BOOKING_CMPID = LEDGERS.Acc_cmpid AND MISCPURMASTER.BOOKING_LOCATIONID = LEDGERS.Acc_locationid AND MISCPURMASTER.BOOKING_YEARID = LEDGERS.Acc_yearid ", " AND ( MISCPURMASTER.BOOKING_PURBILLINITIALS = '" & txtbillno.Text.Trim & "') AND MISCPURMASTER.BOOKING_PURBALANCE > 0  AND (MISCPURMASTER.BOOKING_cmpid = " & CmpId & ") AND (MISCPURMASTER.BOOKING_locationid = " & Locationid & ") AND (MISCPURMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                'CHECKING IN HOLIDAYPACKAGEMASTER
                DT = OBJCMN.search(" TOP 1 HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_PURBILLINITIALS AS BILLINITIALS, HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_DATE AS BILLDATE, HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_BALANCE AS BALAMT, HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_GTOTAL AS BILLAMT, 'PACKAGEPURCHASE' AS BILLTYPE, HOLIDAYPACKAGEMASTER.BOOKING_NO AS BILLNO, REGISTERMASTER.register_name AS REGTYPE, LEDGERS.Acc_cmpname AS NAME ", "", "  LEDGERS INNER JOIN HOLIDAYPACKAGEMASTER_PURCHASEDETAILS ON LEDGERS.Acc_id = HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_PURLEDGERID AND LEDGERS.Acc_cmpid = HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_CMPID AND LEDGERS.Acc_locationid = HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_LOCATIONID AND LEDGERS.Acc_yearid = HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_YEARID RIGHT OUTER JOIN HOLIDAYPACKAGEMASTER INNER JOIN REGISTERMASTER ON HOLIDAYPACKAGEMASTER.BOOKING_PURCHASEREGISTERID = REGISTERMASTER.register_id AND HOLIDAYPACKAGEMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND HOLIDAYPACKAGEMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND HOLIDAYPACKAGEMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid ON HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_NO = HOLIDAYPACKAGEMASTER.BOOKING_NO AND HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_CMPID = HOLIDAYPACKAGEMASTER.BOOKING_CMPID AND HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_LOCATIONID = HOLIDAYPACKAGEMASTER.BOOKING_LOCATIONID AND HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_YEARID = HOLIDAYPACKAGEMASTER.BOOKING_YEARID ", " AND ( HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_PURBILLINITIALS = '" & txtbillno.Text.Trim & "') AND HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_BALANCE > 0  AND (HOLIDAYPACKAGEMASTER.BOOKING_cmpid = " & CmpId & ") AND (HOLIDAYPACKAGEMASTER.BOOKING_locationid = " & Locationid & ") AND (HOLIDAYPACKAGEMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                'CHECKING IN INTERNATIONALBOOKINGMASTER
                DT = OBJCMN.search(" TOP 1 INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_PURBILLINITIALS AS BILLINITIALS, INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_DATE AS BILLDATE, INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_BALANCE AS BALAMT, INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_GTOTAL AS BILLAMT, 'INTPURCHASE' AS BILLTYPE, INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_NO AS BILLNO, REGISTERMASTER.register_name AS REGTYPE, LEDGERS.Acc_cmpname AS NAME ", "", "  LEDGERS INNER JOIN INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS ON LEDGERS.Acc_id = INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_PURLEDGERID AND LEDGERS.Acc_cmpid = INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_CMPID AND LEDGERS.Acc_locationid = INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_LOCATIONID AND LEDGERS.Acc_yearid = INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_YEARID RIGHT OUTER JOIN INTERNATIONALBOOKINGMASTER INNER JOIN REGISTERMASTER ON INTERNATIONALBOOKINGMASTER.BOOKING_PURCHASEREGISTERID = REGISTERMASTER.register_id AND INTERNATIONALBOOKINGMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND INTERNATIONALBOOKINGMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND INTERNATIONALBOOKINGMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid ON INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_YEARID = INTERNATIONALBOOKINGMASTER.BOOKING_YEARID AND INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_LOCATIONID = INTERNATIONALBOOKINGMASTER.BOOKING_LOCATIONID AND INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_CMPID = INTERNATIONALBOOKINGMASTER.BOOKING_CMPID AND INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_NO = INTERNATIONALBOOKINGMASTER.BOOKING_NO ", " AND ( INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_PURBILLINITIALS = '" & txtbillno.Text.Trim & "') AND INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_BALANCE > 0  AND (INTERNATIONALBOOKINGMASTER.BOOKING_cmpid = " & CmpId & ") AND (INTERNATIONALBOOKINGMASTER.BOOKING_locationid = " & Locationid & ") AND (INTERNATIONALBOOKINGMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                'CHECKING IN RAILBOOKING
                DT = OBJCMN.search("RAILBOOKINGMASTER.BOOKING_PURBILLINITIALS AS BILLINITIALS, RAILBOOKINGMASTER.BOOKING_DATE AS BILLDATE, RAILBOOKINGMASTER.BOOKING_PURBALANCE AS BALAMT, RAILBOOKINGMASTER.BOOKING_PURGRANDTOTAL AS BILLAMT, 'RAILPURCHASE' AS BILLTYPE, RAILBOOKINGMASTER.BOOKING_NO AS BILLNO,  REGISTERMASTER.register_name AS REGTYPE, LEDGERS.ACC_CMPNAME AS NAME", "", " RAILBOOKINGMASTER INNER JOIN REGISTERMASTER ON RAILBOOKINGMASTER.BOOKING_PURCHASEREGISTERID = REGISTERMASTER.register_id AND RAILBOOKINGMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND RAILBOOKINGMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND RAILBOOKINGMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON RAILBOOKINGMASTER.BOOKING_PURLEDGERID = LEDGERS.Acc_id AND RAILBOOKINGMASTER.BOOKING_CMPID = LEDGERS.Acc_cmpid AND RAILBOOKINGMASTER.BOOKING_LOCATIONID = LEDGERS.Acc_locationid AND RAILBOOKINGMASTER.BOOKING_YEARID = LEDGERS.Acc_yearid ", " AND ( RAILBOOKINGMASTER.BOOKING_PURBILLINITIALS = '" & txtbillno.Text.Trim & "') AND RAILBOOKINGMASTER.BOOKING_PURBALANCE > 0  AND (RAILBOOKINGMASTER.BOOKING_cmpid = " & CmpId & ") AND (RAILBOOKINGMASTER.BOOKING_locationid = " & Locationid & ") AND (RAILBOOKINGMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                'CHECKING IN CARBOOKINGMASTER
                DT = OBJCMN.search(" TOP 1 CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_PURBILLINITIALS AS BILLINITIALS, CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_DATE AS BILLDATE, CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_BALANCE AS BALAMT, CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_GTOTAL AS BILLAMT, 'VEHICLEPURCHASE' AS BILLTYPE, CARBOOKINGMASTER.BOOKING_NO AS BILLNO, REGISTERMASTER.register_name AS REGTYPE, LEDGERS.Acc_cmpname AS NAME ", "", "  LEDGERS INNER JOIN CARBOOKINGMASTER_PURCHASEDETAILS ON LEDGERS.Acc_id = CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_PURLEDGERID AND LEDGERS.Acc_cmpid = CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_CMPID AND LEDGERS.Acc_locationid = CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_LOCATIONID AND LEDGERS.Acc_yearid = CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_YEARID RIGHT OUTER JOIN CARBOOKINGMASTER INNER JOIN REGISTERMASTER ON CARBOOKINGMASTER.BOOKING_PURCHASEREGISTERID = REGISTERMASTER.register_id AND CARBOOKINGMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND CARBOOKINGMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND CARBOOKINGMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid ON CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_NO = CARBOOKINGMASTER.BOOKING_NO AND CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_CMPID = CARBOOKINGMASTER.BOOKING_CMPID AND CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_LOCATIONID = CARBOOKINGMASTER.BOOKING_LOCATIONID AND CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_YEARID = CARBOOKINGMASTER.BOOKING_YEARID ", " AND ( CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_PURBILLINITIALS = '" & txtbillno.Text.Trim & "') AND CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_BALANCE > 0  AND (CARBOOKINGMASTER.BOOKING_cmpid = " & CmpId & ") AND (CARBOOKINGMASTER.BOOKING_locationid = " & Locationid & ") AND (CARBOOKINGMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                'CHECKING IN AIRLINEBOOKING
                ' CHANGED BOOKING_ARRIVAL TO BOOKING_DATE BY SHOHIN
                DT = OBJCMN.search("AIRLINEBOOKINGMASTER.BOOKING_PURBILLINITIALS AS BILLINITIALS, AIRLINEBOOKINGMASTER.BOOKING_DATE AS BILLDATE, AIRLINEBOOKINGMASTER.BOOKING_PURBALANCE AS BALAMT, AIRLINEBOOKINGMASTER.BOOKING_PURGRANDTOTAL AS BILLAMT, 'AIRLINEPURCHASE' AS BILLTYPE, AIRLINEBOOKINGMASTER.BOOKING_NO AS BILLNO,  REGISTERMASTER.register_name AS REGTYPE, LEDGERS.ACC_CMPNAME AS NAME", "", " AIRLINEBOOKINGMASTER INNER JOIN REGISTERMASTER ON AIRLINEBOOKINGMASTER.BOOKING_PURCHASEREGISTERID = REGISTERMASTER.register_id AND AIRLINEBOOKINGMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND AIRLINEBOOKINGMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND AIRLINEBOOKINGMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON AIRLINEBOOKINGMASTER.BOOKING_PURLEDGERID = LEDGERS.Acc_id AND AIRLINEBOOKINGMASTER.BOOKING_CMPID = LEDGERS.Acc_cmpid AND AIRLINEBOOKINGMASTER.BOOKING_LOCATIONID = LEDGERS.Acc_locationid AND AIRLINEBOOKINGMASTER.BOOKING_YEARID = LEDGERS.Acc_yearid ", " AND ( AIRLINEBOOKINGMASTER.BOOKING_PURBILLINITIALS = '" & txtbillno.Text.Trim & "') AND AIRLINEBOOKINGMASTER.BOOKING_PURBALANCE > 0  AND (AIRLINEBOOKINGMASTER.BOOKING_cmpid = " & CmpId & ") AND (AIRLINEBOOKINGMASTER.BOOKING_locationid = " & Locationid & ") AND (AIRLINEBOOKINGMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                For Each ROW As DataGridViewRow In GRIDBILL.Rows
                    If IsDBNull(ROW.Cells("INVBILLINITIALS").Value) = False Then
    ...
    Private Sub cmbbillno_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmbbillno.KeyPress
        enterkeypress(e, Me)
    End Sub
    ...
    Private Sub cmbbillno_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmbbillno.Validating
        Try
            ...
            If cmbbillno.Text.Trim <> "" Then
                ...
                Next
                If Not BLN Then
                    MsgBox("Invalid Bill No.", MsgBoxStyle.Critical, "TRAVELMATE")
                    cmbbillno.Focus()
                    Exit Sub
                End If
                ...
                'GETTING AMT OF THE SELECTED BILL
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.search(" T.BALANCE AS BALAMT", "", " (SELECT BILL_INITIALS AS BILLINITIALS, BILL_BALANCE AS BALANCE, BILL_CMPID AS CMPID , BILL_LOCATIONID AS LOCATIONID , BILL_YEARID AS YEARID FROM OPENINGBILL UNION ALL SELECT HOTELBOOKINGMASTER.BOOKING_PURBILLINITIALS AS BILLINITIALS, HOTELBOOKINGMASTER.BOOKING_PURBALANCE AS BALANCE, BOOKING_CMPID AS CMPID , BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM HOTELBOOKINGMASTER UNION ALL SELECT RAILBOOKINGMASTER.BOOKING_PURBILLINITIALS AS BILLINITIALS, RAILBOOKINGMASTER.BOOKING_PURBALANCE AS BALANCE, BOOKING_CMPID AS CMPID , BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM RAILBOOKINGMASTER UNION ALL SELECT BOOKING_PURBILLINITIALS AS BILLINITIALS, BOOKING_PURBALANCE AS BALANCE, BOOKING_CMPID AS CMPID , BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM MISCPURMASTER UNION ALL SELECT CARBOOKINGMASTER_PURCHASEDETAILS.BOOKING_PURBILLINITIALS AS BILLINITIALS, BOOKING_BALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM CARBOOKINGMASTER_PURCHASEDETAILS UNION ALL SELECT AIRLINEBOOKINGMASTER.BOOKING_PURBILLINITIALS AS BILLINITIALS, AIRLINEBOOKINGMASTER.BOOKING_PURBALANCE AS BALANCE, BOOKING_CMPID AS CMPID , BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM AIRLINEBOOKINGMASTER   UNION ALL SELECT HOLIDAYPACKAGEMASTER_PURCHASEDETAILS.BOOKING_PURBILLINITIALS AS BILLINITIALS, BOOKING_BALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM HOLIDAYPACKAGEMASTER_PURCHASEDETAILS UNION ALL SELECT INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS.BOOKING_PURBILLINITIALS AS BILLINITIALS, BOOKING_BALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID , BOOKING_YEARID AS YEARID FROM INTERNATIONALBOOKINGMASTER_PURCHASEDETAILS  UNION ALL	SELECT JOURNALMASTER.JOURNAL_INITIALS AS BILLINITIALS, (SUM(JOURNAL_DEBIT)-(JOURNAL_AMT + JOURNAL_TDS)) AS BALANCE, JOURNAL_CMPID AS CMPID, JOURNAL_LOCATIONID AS LOCATIONID , JOURNAL_YEARID AS YEARID FROM JOURNALMASTER GROUP BY journal_initials,journal_amt, journal_tds, JOURNAL_CMPID, JOURNAL_LOCATIONID, JOURNAL_YEARID UNION ALL	SELECT NONPURCHASE.NP_INITIALS AS BILLINITIALS, NP_BALANCE AS BALANCE, NP_CMPID AS CMPID, NP_LOCATIONID AS LOCATIONID , NP_YEARID AS YEARID  FROM NONPURCHASE UNION ALL SELECT SAL_INITIALS AS BILLINITIALS, SAL_BALANCE AS BALANCE, SAL_CMPID AS CMPID , SAL_LOCATIONID AS LOCATIONID , SAL_YEARID AS YEARID FROM SALARYMASTER UNION ALL SELECT CAST(RECEIPTMASTER_DESC.RECEIPT_GRIDREMARKS AS VARCHAR(100)) AS BILLINITIALS, RECEIPTMASTER_DESC.RECEIPT_BALANCE AS BALANCE, RECEIPT_CMPID AS CMPID , 0 AS LOCATIONID , RECEIPT_YEARID AS YEARID FROM RECEIPTMASTER_DESC WHERE RECEIPT_paytype = 'New Ref' AND ISNULL(RECEIPT_GRIDREMARKS,'')  <> '') AS T", " AND T.BILLINITIALS = '" & cmbbillno.Text.Trim & "' AND T.CMPID = " & CmpId & " AND T.LOCATIONID = " & Locationid & " AND T.YEARID = " & YearId)
                If DT.Rows.Count > 0 Then
                    lblbilltotal.Text = Format(DT.Rows(0).Item("BALAMT"), "0.00")
                End If
        ...
        Try
            ...
            If txtsrno.Text.Trim.Length > 0 And Val(txtamt.Text) > 0 Then
                If cmbpaytype.Text = "Against Bill" And cmbbillno.Text.Trim = "" Then
                    MsgBox("Select Bill First", MsgBoxStyle.Critical, "TRAVELMATE")
                    cmbpaytype.Focus()
                    Exit Sub
                End If
                ...
                If cmbbillno.Text.Trim <> "" Then
                    For Each ROW As DataGridViewRow In gridpayment.Rows
                        If (ROW.Cells(gbillno.Index).Value = cmbbillno.Text.Trim And GRIDDOUBLECLICK = False) Or (GRIDDOUBLECLICK = True And ROW.Cells(gbillno.Index).Value = cmbbillno.Text.Trim And ROW.Index <> temprow) Then
                            MsgBox("Bill Already present in Grid below", MsgBoxStyle.Critical, "TRAVELMATE")
                            cmbpaytype.Focus()
                            Exit Sub
                        End If
                    Next
    ...
    Private Sub ToolStripdelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripdelete.Click
        Try
            If edit = True Then
                ...
                'checking whether recd is made or not
                For Each row As DataGridViewRow In gridpayment.Rows
                    If row.Cells(GAMTREC.Index).Value > 0 Or row.Cells(GEXTRAAMT.Index).Value > 0 Or row.Cells(GRETURN.Index).Value > 0 Then
                        MsgBox("Payment Locked, Delete Rec First", MsgBoxStyle.Critical)
                        Exit Sub
                    End If
                Next

                Dim tempmsg As Integer = MsgBox("Delete Payment Entry Permanently?", MsgBoxStyle.YesNo, "TRAVELMATE")
                If tempmsg = vbYes Then
    ...
    Private Sub txtchqno_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTCHQNO.Validating
        Try
            If TXTCHQNO.Text.Trim <> "" And ClientName <> "SSR" Then
                ...
                If DT.Rows.Count > 0 Then
                    If (edit = False) Or (edit = True And CHQNO <> TXTCHQNO.Text.Trim) Then
                        MsgBox("Chq. No. Already Present with this Bank in Payment No." & DT.Rows(0).Item(0), MsgBoxStyle.Critical, "TRAVELMATE")
                        e.Cancel = True
                    End If
                End If
            End If
    ...
    Private Sub gridpaydesc_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridpaydesc.CellDoubleClick
        Try
            If e.RowIndex >= 0 And gridpaydesc.Item(gridsrno.Index, e.RowIndex).Value <> Nothing Then
                ...
                If gridpayment.Rows(gridpaydesc.Item(PAYGRIDSRNO.Index, e.RowIndex).Value).Cells(GAMTREC.Index).Value > 0 Or gridpayment.Rows(gridpaydesc.Item(PAYGRIDSRNO.Index, e.RowIndex).Value).Cells(GEXTRAAMT.Index).Value > 0 Or gridpayment.Rows(gridpaydesc.Item(PAYGRIDSRNO.Index, e.RowIndex).Value).Cells(GRETURN.Index).Value > 0 Then
                    MsgBox("Payment Locked, Delete Rec First", MsgBoxStyle.Critical)
                    Exit Sub
                End If
    ...
    Private Sub gridpaydesc_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles gridpaydesc.KeyDown
        Try
            If e.KeyCode = Keys.Delete Then
                ...
                If gridpayment.Rows(gridpaydesc.Item(PAYGRIDSRNO.Index, gridpaydesc.CurrentRow.Index).Value).Cells(GAMTREC.Index).Value > 0 Or gridpayment.Rows(gridpaydesc.Item(PAYGRIDSRNO.Index, gridpaydesc.CurrentRow.Index).Value).Cells(GEXTRAAMT.Index).Value > 0 Or gridpayment.Rows(gridpaydesc.Item(PAYGRIDSRNO.Index, gridpaydesc.CurrentRow.Index).Value).Cells(GRETURN.Index).Value > 0 Then
                    MsgBox("Payment Locked, Delete Rec First", MsgBoxStyle.Critical)
                    Exit Sub
                End If
...
1:
    ...
    Private Sub txtnarr_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtnarr.Validating
        Try
            If cmbpaytype.Text = "New Ref" And txtnarr.Text.Trim = "" Then
                MsgBox("Enter Ref No", MsgBoxStyle.Critical)
                e.Cancel = True
            ElseIf cmbpaytype.Text.Trim = "New Ref" And txtnarr.Text.Trim <> "" Then
                'checking WHETEHER REFNO IS ALREADY PRESENT OR NOT
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable
                Dim WHERECLAUSE As String = " AND PAYMENT_CMPID =  " & CmpId & " AND PAYMENT_YEARID = " & YearId & " AND CAST(PAYMENT_GRIDREMARKS AS VARCHAR(MAX)) = '" & txtnarr.Text.Trim & "' "
                If edit = True Then WHERECLAUSE = WHERECLAUSE & " AND PAYMENT_NO <> " & TEMPPAYMENTNO

                DT = OBJCMN.search(" PAYMENT_NO AS PAYNO ", "", " PAYMENTMASTER_DESC ", WHERECLAUSE)
                If DT.Rows.Count > 0 Then
                    MsgBox("Ref no already Present in Payment No " & DT.Rows(0).Item("PAYNO"), MsgBoxStyle.Critical)
                    e.Cancel = True
                End If


                'CHECKING IN GRID BELOW
                For Each ROW As DataGridViewRow In gridpayment.Rows
                    If GRIDDOUBLECLICK = False Then
                        If ROW.Cells(gdesc.Index).Value = txtnarr.Text.Trim Then
                            MsgBox("Ref no already Present in Grid below", MsgBoxStyle.Critical)
                            e.Cancel = True
                        End If
                    Else
                        If ROW.Cells(gdesc.Index).Value = txtnarr.Text.Trim And ROW.Index <> temprow Then
                            MsgBox("Ref no already Present in Grid below", MsgBoxStyle.Critical)
                            e.Cancel = True
                        End If
                    End If
    ...
    Sub SMSCODE()
        If ClientName = "TRINITY" And txtmobileno.Text.Trim <> "" Then
            If TXTCHQNO.Text.Trim <> "" Then
                If SENDMSG("Hi, Payment of Rs. " & Format(Val(txtchqamt.Text.Trim), "0.00") & " has been debited in your account. Cheque No : " & TXTCHQNO.Text.Trim & ", Bank : " & cmbaccname.Text.Trim & ", Dt. " & Format(ACCDATE.Value.Date, "dd/MM/yyyy"), txtmobileno.Text.Trim) = "1701" Then MsgBox("Message Sent") Else MsgBox("Error Sending Message")
            Else
                If SENDMSG("Hi, Payment of Rs. " & Format(Val(txtchqamt.Text.Trim), "0.00") & " has been debited in your account.", txtmobileno.Text.Trim) = "1701" Then MsgBox("Message Sent") Else MsgBox("Error Sending Message")
            End If
        End If
    End Sub
End Class