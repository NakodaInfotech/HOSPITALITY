...
Public Class Receipt
    ...
    Function ERRORVALID() As Boolean
        Try
            ...
            'IF RECO IS DONE THEN DONT ALLOW TOM CHANGE THE DATE AND AMOUNT, USER CAN CHANGE THE NAME
    ...
    Private Sub Receipt_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        ElseIf e.Alt = True And e.KeyCode = Windows.Forms.Keys.F1 Then
            Dim OBJINVDTLS As New ReceiptDetails
    ...
    Private Sub cmbname_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbname.Validated
        Try
            ...
            If cmbname.Text.Trim <> "" Then GETBALANCE()
    ...
    Private Sub cmdsave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdsave.Click
        Try
            ...
            'GET BILLREMARKS
            If edit = False And ClientName = "URMI" Then
                For Each ROW As DataGridViewRow In gridpayment.Rows
                    If ROW.Cells(gpaytype.Index).Value = "Against Bill" Then
                        If txtremarks.Text = "" Then
                            txtremarks.Text = "Bg. Chqno. " & txtchqno.Text.Trim & " of " & TXTBANKNAME.Text.Trim & " Against Bill No : " & ROW.Cells(gbillno.Index).Value
                        Else
                            txtremarks.Text = txtremarks.Text & ", " & ROW.Cells(gbillno.Index).Value
                        End If
                    End If
                Next
            End If
        ...
        Try
            ...
            If e.RowIndex >= 0 And gridpayment.Item(gridsrno.Index, e.RowIndex).Value <> Nothing Then
                ...
                If cmbbillno.Text.Trim <> "" Then
                    ...
                    Dim OBJCMN As New ClsCommon
                    Dim DT As DataTable = OBJCMN.search(" T.BALANCE AS BALANCE", "", " (SELECT BILL_INITIALS AS BILLINITIALS, BILL_BALANCE AS BALANCE, BILL_CMPID AS CMPID , BILL_LOCATIONID AS LOCATIONID , BILL_YEARID AS YEARID FROM OPENINGBILL UNION ALL SELECT BOOKING_SALEBILLINITIALS AS BILLINITIALS, BOOKING_SALEBALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID, BOOKING_YEARID AS YEARID FROM HOTELBOOKINGMASTER UNION ALL SELECT BOOKING_SALEBILLINITIALS AS BILLINITIALS, BOOKING_SALEBALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID, BOOKING_YEARID AS YEARID FROM RAILBOOKINGMASTER UNION  ALL SELECT BOOKING_SALEBILLINITIALS AS BILLINITIALS, BOOKING_SALEBALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID, BOOKING_YEARID AS YEARID FROM CARBOOKINGMASTER UNION ALL SELECT BOOKING_PURBILLINITIALS AS BILLINITIALS, BOOKING_PURBALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID, BOOKING_YEARID AS YEARID FROM MISCSALMASTER UNION ALL SELECT BOOKING_SALEBILLINITIALS AS BILLINITIALS, BOOKING_SALEBALANCE AS BALANCE, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID, BOOKING_YEARID AS YEARID FROM AIRLINEBOOKINGMASTER UNION ALL SELECT BOOKING_SALEBILLINITIALS AS BILLINITIALS, BOOKING_SALEBALANCE AS BALAMT, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID, BOOKING_YEARID AS YEARID FROM HOLIDAYPACKAGEMASTER UNION ALL SELECT BOOKING_SALEBILLINITIALS AS BILLINITIALS, BOOKING_SALEBALANCE AS BALAMT, BOOKING_CMPID AS CMPID, BOOKING_LOCATIONID AS LOCATIONID, BOOKING_YEARID AS YEARID FROM INTERNATIONALBOOKINGMASTER UNION ALL	SELECT JOURNALMASTER.JOURNAL_INITIALS AS BILLINITIALS, (SUM(JOURNAL_DEBIT)-(JOURNAL_AMT + JOURNAL_TDS)) AS BALANCE, JOURNAL_CMPID AS CMPID, JOURNAL_LOCATIONID AS LOCATIONID , JOURNAL_YEARID AS YEARID FROM JOURNALMASTER GROUP BY journal_initials,journal_amt, journal_tds, JOURNAL_CMPID, JOURNAL_LOCATIONID, JOURNAL_YEARID UNION ALL	SELECT NONPURCHASE.NP_INITIALS AS BILLINITIALS, NP_BALANCE AS BALANCE, NP_CMPID AS CMPID, NP_LOCATIONID AS LOCATIONID , NP_YEARID AS YEARID  FROM NONPURCHASE UNION ALL	SELECT CAST(PAYMENT_GRIDREMARKS AS VARCHAR(100)) AS BILLINITIALS, PAYMENT_BALANCE AS BALANCE, PAYMENT_CMPID AS CMPID, PAYMENT_LOCATIONID AS LOCATIONID , PAYMENT_YEARID AS YEARID  FROM PAYMENTMASTER_DESC WHERE PAYMENT_PAYTYPE = 'New Ref' AND ISNULL(PAYMENT_GRIDREMARKS,'') <> '') AS T", " AND T.BILLINITIALS = '" & cmbbillno.Text.Trim & "' AND T.CMPID = " & CmpId & " AND T.LOCATIONID = " & Locationid & " AND T.YEARID = " & YearId)
                    If DT.Rows.Count > 0 Then
...
LINE1:
    ...
    Private Sub cmbaccname_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmbaccname.Validating
        Try
            If cmbaccname.Text.Trim <> "" Then ledgervalidate(cmbaccname, CMBACCCODE, e, Me, txtadd, " AND (GROUPMASTER.group_SECONDARY = 'BANK A/C' OR GROUPMASTER.group_SECONDARY = 'BANK OD A/C' OR GROUPMASTER.group_SECONDARY = 'CASH IN HAND') AND ACC_YEARID = " & YearId)
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Private Sub txtbillno_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtbillno.Validating
        Try
            ...
            If txtbillno.Text.Trim <> "" Then
                ...
                DT = OBJCMN.search("HOTELBOOKINGMASTER.BOOKING_SALEBILLINITIALS AS BILLINITIALS, HOTELBOOKINGMASTER.BOOKING_DATE AS BILLDATE, HOTELBOOKINGMASTER.BOOKING_SALEBALANCE AS BALAMT, HOTELBOOKINGMASTER.BOOKING_GRANDTOTAL AS BILLAMT, 'INVOICE' AS BILLTYPE, HOTELBOOKINGMASTER.BOOKING_NO AS BILLNO,  REGISTERMASTER.register_name AS REGTYPE, LEDGERS.ACC_CMPNAME AS NAME", "", " HOTELBOOKINGMASTER INNER JOIN REGISTERMASTER ON HOTELBOOKINGMASTER.BOOKING_SALEREGISTERID = REGISTERMASTER.register_id AND HOTELBOOKINGMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND HOTELBOOKINGMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND HOTELBOOKINGMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON HOTELBOOKINGMASTER.BOOKING_LEDGERID = LEDGERS.Acc_id AND HOTELBOOKINGMASTER.BOOKING_CMPID = LEDGERS.Acc_cmpid AND HOTELBOOKINGMASTER.BOOKING_LOCATIONID = LEDGERS.Acc_locationid AND HOTELBOOKINGMASTER.BOOKING_YEARID = LEDGERS.Acc_yearid ", " AND ( HOTELBOOKINGMASTER.BOOKING_SALEBILLINITIALS = '" & txtbillno.Text.Trim & "') AND HOTELBOOKINGMASTER.BOOKING_SALEBALANCE > 0  AND (HOTELBOOKINGMASTER.BOOKING_cmpid = " & CmpId & ") AND (HOTELBOOKINGMASTER.BOOKING_locationid = " & Locationid & ") AND (HOTELBOOKINGMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                'CHECKING IN RAILBOOKINGMASTER
                DT = OBJCMN.search("BOOKING_SALEBILLINITIALS AS BILLINITIALS, BOOKING_DATE AS BILLDATE, BOOKING_SALEBALANCE AS BALAMT, BOOKING_GRANDTOTAL AS BILLAMT, 'RAILINVOICE' AS BILLTYPE, BOOKING_NO AS BILLNO,  REGISTERMASTER.register_name AS REGTYPE, LEDGERS.ACC_CMPNAME AS NAME", "", " RAILBOOKINGMASTER INNER JOIN REGISTERMASTER ON RAILBOOKINGMASTER.BOOKING_SALEREGISTERID = REGISTERMASTER.register_id AND RAILBOOKINGMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND RAILBOOKINGMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND RAILBOOKINGMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON RAILBOOKINGMASTER.BOOKING_LEDGERID = LEDGERS.Acc_id AND RAILBOOKINGMASTER.BOOKING_CMPID = LEDGERS.Acc_cmpid AND RAILBOOKINGMASTER.BOOKING_LOCATIONID = LEDGERS.Acc_locationid AND RAILBOOKINGMASTER.BOOKING_YEARID = LEDGERS.Acc_yearid ", " AND ( RAILBOOKINGMASTER.BOOKING_SALEBILLINITIALS = '" & txtbillno.Text.Trim & "') AND RAILBOOKINGMASTER.BOOKING_SALEBALANCE > 0  AND (RAILBOOKINGMASTER.BOOKING_cmpid = " & CmpId & ") AND (RAILBOOKINGMASTER.BOOKING_locationid = " & Locationid & ") AND (RAILBOOKINGMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                DT = OBJCMN.search(" TOP 1  HOLIDAYPACKAGEMASTER.BOOKING_SALEBILLINITIALS AS BILLINITIALS, HOLIDAYPACKAGEMASTER_DESC.BOOKING_ARRIVALDATE AS BILLDATE, HOLIDAYPACKAGEMASTER.BOOKING_SALEBALANCE AS BALAMT, HOLIDAYPACKAGEMASTER.BOOKING_GRANDTOTAL AS BILLAMT, 'PACKAGEINVOICE' AS BILLTYPE, HOLIDAYPACKAGEMASTER.BOOKING_NO AS BILLNO, REGISTERMASTER.register_name AS REGTYPE, LEDGERS.Acc_cmpname AS NAME", "", "  HOLIDAYPACKAGEMASTER INNER JOIN REGISTERMASTER ON HOLIDAYPACKAGEMASTER.BOOKING_SALEREGISTERID = REGISTERMASTER.register_id AND HOLIDAYPACKAGEMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND HOLIDAYPACKAGEMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND HOLIDAYPACKAGEMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON HOLIDAYPACKAGEMASTER.BOOKING_CMPID = LEDGERS.Acc_cmpid AND HOLIDAYPACKAGEMASTER.BOOKING_LOCATIONID = LEDGERS.Acc_locationid AND HOLIDAYPACKAGEMASTER.BOOKING_YEARID = LEDGERS.Acc_yearid AND HOLIDAYPACKAGEMASTER.BOOKING_LEDGERID = LEDGERS.Acc_id INNER JOIN HOLIDAYPACKAGEMASTER_DESC ON HOLIDAYPACKAGEMASTER.BOOKING_NO = HOLIDAYPACKAGEMASTER_DESC.BOOKING_NO AND HOLIDAYPACKAGEMASTER.BOOKING_CMPID = HOLIDAYPACKAGEMASTER_DESC.BOOKING_CMPID AND HOLIDAYPACKAGEMASTER.BOOKING_LOCATIONID = HOLIDAYPACKAGEMASTER_DESC.BOOKING_LOCATIONID AND HOLIDAYPACKAGEMASTER.BOOKING_YEARID = HOLIDAYPACKAGEMASTER_DESC.BOOKING_YEARID ", " AND ( HOLIDAYPACKAGEMASTER.BOOKING_SALEBILLINITIALS = '" & txtbillno.Text.Trim & "') AND HOLIDAYPACKAGEMASTER.BOOKING_SALEBALANCE > 0  AND (HOLIDAYPACKAGEMASTER.BOOKING_cmpid = " & CmpId & ") AND (HOLIDAYPACKAGEMASTER.BOOKING_locationid = " & Locationid & ") AND (HOLIDAYPACKAGEMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                DT = OBJCMN.search(" TOP 1  INTERNATIONALBOOKINGMASTER.BOOKING_SALEBILLINITIALS AS BILLINITIALS, INTERNATIONALBOOKINGMASTER_DESC.BOOKING_ARRIVALDATE AS BILLDATE, INTERNATIONALBOOKINGMASTER.BOOKING_SALEBALANCE AS BALAMT, INTERNATIONALBOOKINGMASTER.BOOKING_GRANDTOTAL AS BILLAMT, 'INTINVOICE' AS BILLTYPE, INTERNATIONALBOOKINGMASTER.BOOKING_NO AS BILLNO, REGISTERMASTER.register_name AS REGTYPE, LEDGERS.Acc_cmpname AS NAME", "", "  INTERNATIONALBOOKINGMASTER INNER JOIN REGISTERMASTER ON INTERNATIONALBOOKINGMASTER.BOOKING_SALEREGISTERID = REGISTERMASTER.register_id AND INTERNATIONALBOOKINGMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND INTERNATIONALBOOKINGMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND INTERNATIONALBOOKINGMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON INTERNATIONALBOOKINGMASTER.BOOKING_CMPID = LEDGERS.Acc_cmpid AND INTERNATIONALBOOKINGMASTER.BOOKING_LOCATIONID = LEDGERS.Acc_locationid AND INTERNATIONALBOOKINGMASTER.BOOKING_YEARID = LEDGERS.Acc_yearid AND INTERNATIONALBOOKINGMASTER.BOOKING_LEDGERID = LEDGERS.Acc_id INNER JOIN INTERNATIONALBOOKINGMASTER_DESC ON INTERNATIONALBOOKINGMASTER.BOOKING_NO = INTERNATIONALBOOKINGMASTER_DESC.BOOKING_NO AND INTERNATIONALBOOKINGMASTER.BOOKING_CMPID = INTERNATIONALBOOKINGMASTER_DESC.BOOKING_CMPID AND INTERNATIONALBOOKINGMASTER.BOOKING_LOCATIONID = INTERNATIONALBOOKINGMASTER_DESC.BOOKING_LOCATIONID AND INTERNATIONALBOOKINGMASTER.BOOKING_YEARID = INTERNATIONALBOOKINGMASTER_DESC.BOOKING_YEARID ", " AND ( INTERNATIONALBOOKINGMASTER.BOOKING_SALEBILLINITIALS = '" & txtbillno.Text.Trim & "') AND INTERNATIONALBOOKINGMASTER.BOOKING_SALEBALANCE > 0  AND (INTERNATIONALBOOKINGMASTER.BOOKING_cmpid = " & CmpId & ") AND (INTERNATIONALBOOKINGMASTER.BOOKING_locationid = " & Locationid & ") AND (INTERNATIONALBOOKINGMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
                ...
                DT = OBJCMN.search("AIRLINEBOOKINGMASTER.BOOKING_SALEBILLINITIALS AS BILLINITIALS, AIRLINEBOOKINGMASTER.BOOKING_ARRIVAL AS BILLDATE, AIRLINEBOOKINGMASTER.BOOKING_SALEBALANCE AS BALAMT, AIRLINEBOOKINGMASTER.BOOKING_GRANDTOTAL AS BILLAMT, 'INVOICE' AS BILLTYPE, AIRLINEBOOKINGMASTER.BOOKING_NO AS BILLNO,  REGISTERMASTER.register_name AS REGTYPE, LEDGERS.ACC_CMPNAME AS NAME", "", " AIRLINEBOOKINGMASTER INNER JOIN REGISTERMASTER ON AIRLINEBOOKINGMASTER.BOOKING_SALEREGISTERID = REGISTERMASTER.register_id AND AIRLINEBOOKINGMASTER.BOOKING_CMPID = REGISTERMASTER.register_cmpid AND AIRLINEBOOKINGMASTER.BOOKING_LOCATIONID = REGISTERMASTER.register_locationid AND AIRLINEBOOKINGMASTER.BOOKING_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON AIRLINEBOOKINGMASTER.BOOKING_LEDGERID = LEDGERS.Acc_id AND AIRLINEBOOKINGMASTER.BOOKING_CMPID = LEDGERS.Acc_cmpid AND AIRLINEBOOKINGMASTER.BOOKING_LOCATIONID = LEDGERS.Acc_locationid AND AIRLINEBOOKINGMASTER.BOOKING_YEARID = LEDGERS.Acc_yearid ", " AND ( AIRLINEBOOKINGMASTER.BOOKING_SALEBILLINITIALS = '" & txtbillno.Text.Trim & "') AND AIRLINEBOOKINGMASTER.BOOKING_SALEBALANCE > 0  AND (AIRLINEBOOKINGMASTER.BOOKING_cmpid = " & CmpId & ") AND (AIRLINEBOOKINGMASTER.BOOKING_locationid = " & Locationid & ") AND (AIRLINEBOOKINGMASTER.BOOKING_yearid = " & YearId & ")")
                If DT.Rows.Count > 0 Then
    ...
    Private Sub ToolStripdelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripdelete.Click
        Try
            If edit = True Then
                ...
                Dim tempmsg As Integer = MsgBox("Delete Receipt Entry Permanently?", MsgBoxStyle.YesNo, "TRAVELMATE")
                If tempmsg = vbYes Then
    ...
    Private Sub txtchqno_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtchqno.Validating
        Try
            If txtchqno.Text.Trim <> "" And ClientName <> "SSR" Then
                ...
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.search(" RECEIPT_INITIALS", "", " RECEIPTMASTER INNER JOIN LEDGERS ON ACC_ID = RECEIPT_ACCID", " AND RECEIPT_CHQNO = '" & txtchqno.Text.Trim & "' AND ACC_CMPNAME = '" & cmbaccname.Text.Trim & "' AND RECEIPT_YEARID = " & YearId)
                If DT.Rows.Count > 0 Then
                    If (edit = False) Or (edit = True And CHQNO <> txtchqno.Text.Trim) Then
                        MsgBox("Chq. No. Already Present with this Bank in Receipt No." & DT.Rows(0).Item(0), MsgBoxStyle.Critical, "TRAVELMATE")
                        e.Cancel = True
                    End If
                End If
            End If
...
1:
    ...
    Private Sub Receipt_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        Try
            ...
            If ClientName = "ELYSIUM" And TEMPAUTOENTRY = True Then
                Dim A As System.ComponentModel.CancelEventArgs
                cmbname.Focus()
    ...
    Private Sub CMDVIEW_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMDVIEW.Click
        Try
            If ClientName = "ELYSIUM" Then
                ...
                SELECTEDBILLNO = OBJSELECTBILL.BILLNO

                If SELECTEDBILLNO.Trim <> "" Then
                    Dim OBJCMN As New ClsCommon
                    Dim DT As DataTable = OBJCMN.search(" LEDGERS.ACC_CMPNAME AS NAME", "", " HOTELBOOKINGMASTER INNER JOIN LEDGERS ON HOTELBOOKINGMASTER.BOOKING_LEDGERID = LEDGERS.Acc_id AND HOTELBOOKINGMASTER.BOOKING_CMPID = LEDGERS.Acc_cmpid AND HOTELBOOKINGMASTER.BOOKING_LOCATIONID = LEDGERS.Acc_locationid AND HOTELBOOKINGMASTER.BOOKING_YEARID = LEDGERS.Acc_yearid ", " AND HOTELBOOKINGMASTER.BOOKING_SALEBILLINITIALS = '" & SELECTEDBILLNO & "' AND HOTELBOOKINGMASTER.BOOKING_CMPID = " & CmpId & " AND HOTELBOOKINGMASTER.BOOKING_LOCATIONID = " & Locationid & " AND HOTELBOOKINGMASTER.BOOKING_YEARID =" & YearId)
                    If DT.Rows.Count > 0 Then
                        cmbname.Text = DT.Rows(0).Item("NAME")
    ...
    Sub SMSCODE()
        If ClientName = "TRINITY" And txtmobileno.Text.Trim <> "" Then
            If txtchqno.Text.Trim <> "" And TXTBANKNAME.Text.Trim <> "" Then
                If SENDMSG("Thanks for your Payment. Rs. " & Format(Val(txtchqamt.Text.Trim), "0.00") & " has been Credited in your account. Cheque No : " & txtchqno.Text.Trim & ", Bank : " & TXTBANKNAME.Text.Trim, txtmobileno.Text.Trim) = "1701" Then MsgBox("Message Sent") Else MsgBox("Error Sending Message")
            Else
                If SENDMSG("Thanks for your Payment. Rs. " & Format(Val(txtchqamt.Text.Trim), "0.00") & " has been Credited in your account.", txtmobileno.Text.Trim) = "1701" Then MsgBox("Message Sent") Else MsgBox("Error Sending Message")
            End If
        End If
    End Sub
End Class