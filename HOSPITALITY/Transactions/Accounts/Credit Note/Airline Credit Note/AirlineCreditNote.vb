...
Public Class AirlineCreditNote
    ...
    Sub TOTAL()
        ...
        For Each ROW As DataGridViewRow In GRIDBOOKINGS.Rows
            If Convert.ToBoolean(ROW.Cells(GPASSCHK.Index).Value) = True Then
                ...
                TXTTOTALSALEAMT.Text = Format(Val(TXTTOTALSALEAMT.Text.Trim) + Val(ROW.Cells(GTOTAL.Index).Value), "0.00")
                rowcount = rowcount + 1
            End If
        Next

        Dim OBJCMN As New ClsCommon
        Dim DT As DataTable = OBJCMN.search(" BOOKING_TOTALPASS AS ACTUALPAX, BOOKING_SALEDISCAMT AS DISCAMT, BOOKING_SALEADDDISCAMT AS ADDDISC", "", " AIRLINEBOOKINGMASTER", " AND BOOKING_SALEBILLINITIALS = '" & TXTBILLNO.Text.Trim & "' AND BOOKING_CMPID = " & CmpId & " AND BOOKING_LOCATIONID = " & Locationid & " AND BOOKING_YEARID = " & YearId)
        ...
        If Val(TXTTDSPER.Text) > 0 Then
            TXTTDSRS.Text = Format((Val(TXTTDSPER.Text) * Val(TXTDISCRS.Text)) / 100, "0.00")
        End If


        TXTSUBTOTAL.Text = Format((Val(TXTTOTALSALEAMT.Text) - Val(TXTDISCRS.Text)) + Val(TXTEXTRACHGS.Text.Trim), "0.00")
        'TXTSUBTOTAL.Text = Format(Val(Val(TXTTOTALSALEAMT.Text) - Val(TXTDISCRS.Text) - Val(TXTADDDISC.Text)), "0.00")
        ...
        'If dt.Rows.Count > 0 Then txttax.Text = Format((Val(dt.Rows(0).Item(1)) * (Val(TXTTOTALBASIC.Text))) / 100, "0.00")


        'If cmbaddsub.Text = "Add" Then
        '    txtgrandtotal.Text = Format(Val(TXTSUBTOTAL.Text) + Val(txttax.Text) + Val(txtotherchg.Text) - Val(TXTCANCEL.Text.Trim), "0")
        '    txtroundoff.Text = Format(Val(txtgrandtotal.Text) - (Val(TXTSUBTOTAL.Text) + Val(txttax.Text) + Val(txtotherchg.Text) - Val(TXTCANCEL.Text.Trim)), "0.00")
        'Else
        '    txtgrandtotal.Text = Format((Val(TXTSUBTOTAL.Text) + Val(txttax.Text)) - Val(txtotherchg.Text) - Val(TXTCANCEL.Text.Trim), "0")
        '    txtroundoff.Text = Format(Val(txtgrandtotal.Text) - ((Val(TXTSUBTOTAL.Text) + Val(txttax.Text)) - Val(txtotherchg.Text) - Val(TXTCANCEL.Text.Trim)), "0.00")
        'End If
        ...
        If cmbaddsub.Text = "Add" Then
            txtgrandtotal.Text = Format(Val(TXTSUBTOTAL.Text) + Val(txttax.Text) + Val(TXTCGSTAMT.Text) + Val(TXTSGSTAMT.Text) + Val(TXTIGSTAMT.Text) - Val(TXTADDDISC.Text) + Val(txtotherchg.Text) - Val(TXTCANCEL.Text.Trim), "0")
            txtroundoff.Text = Format(Val(txtgrandtotal.Text) - (Val(TXTSUBTOTAL.Text) + Val(txttax.Text) + Val(TXTCGSTAMT.Text) + Val(TXTSGSTAMT.Text) + Val(TXTIGSTAMT.Text) - Val(TXTADDDISC.Text) + Val(txtotherchg.Text) - Val(TXTCANCEL.Text.Trim)), "0.00")
        Else
            txtgrandtotal.Text = Format((Val(TXTSUBTOTAL.Text) + Val(txttax.Text) + Val(TXTCGSTAMT.Text) + Val(TXTSGSTAMT.Text) + Val(TXTIGSTAMT.Text) - Val(TXTADDDISC.Text)) - Val(txtotherchg.Text) - Val(TXTCANCEL.Text.Trim), "0")
            txtroundoff.Text = Format(Val(txtgrandtotal.Text) - ((Val(TXTSUBTOTAL.Text) + Val(txttax.Text) + Val(TXTCGSTAMT.Text) + Val(TXTSGSTAMT.Text) + Val(TXTIGSTAMT.Text) - Val(TXTADDDISC.Text)) - Val(txtotherchg.Text) - Val(TXTCANCEL.Text.Trim)), "0.00")
        End If
        ...
        If ClientName = "SKYMAPS" Then
            txtgrandtotal.Text = Format((Val(txtgrandtotal.Text) + Val(TXTTDSRS.Text)), "0.00")
        End If
    ...
    Sub getmaxno_CN()
        ...
        DTTABLE = getmax(" isnull(max(AIRCN_NO),0) + 1 ", "AIRCREDITNOTEMASTER", " AND AIRCN_cmpid=" & CmpId & " AND AIRCN_LOCATIONID = " & Locationid & " AND AIRCN_YEARID = " & YearId)
    ...
    Private Sub cmdok_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdok.Click
        ...
        Try
            ...
            alParaval.Add("AIRLINE CREDITNOTE")
            alParaval.Add(TYPE)
            ...
            If TXTBILLNO.Text.Trim = "" Then
                alParaval.Add(CNDATE.Value)
            Else
                alParaval.Add(TICKETDATE.Value)
            End If
            ...
            alParaval.Add(Val(TXTCANCELINC.Text.Trim))
            ...
            Dim OBJAIRCN As New ClsAirCreditNote()
            OBJAIRCN.alParaval = alParaval
            ...
            '***************** DEBIT NOTE CODE ****************************
            If TXTBILLNO.Text.Trim <> "" And edit = False And TYPE = "AIRLINEBOOKING" Then
                ...
                If TEMPMSG = vbYes Then
                    ...
                    OBJDN.AUTOCREATE = True
                    OBJDN.TEMPCNNO = "ACN-" & DTTABLE.Rows(0).Item(0)
                    OBJDN.Show()
                End If
            End If
            '***************** DEBIT NOTE CODE ****************************
    ...
    Private Sub TXTSALETDSPER_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TXTTDSPER.KeyPress, TXTTDSRS.KeyPress
        numdotkeypress(e, sender, Me)
    End Sub

    Private Sub TXTSALETDSPER_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTTDSPER.Validating, TXTTDSRS.Validating
        Try
            TOTAL()
        Catch ex As Exception
            Throw ex
    ...
    Private Function errorvalid() As Boolean
        ...
        'DONE BY GULKIT COZ SAME BILLNO CAN BE USED WITH DIFFERENT TICKETNO
        'If TXTBILLNO.Text.Trim <> "" And edit = False Then
        '    Dim OBJCMN As New ClsCommon
        '    Dim DT As DataTable = OBJCMN.search(" BOOKING_SALERETURN AS SALERETURN", "", " AIRLINEBOOKINGMASTER", " AND BOOKING_SALEBILLINITIALS = '" & TXTBILLNO.Text.Trim & "'  AND BOOKING_CMPID = " & CmpId & " AND BOOKING_LOCATIONID = " & Locationid & " AND BOOKING_YEARID = " & YearId)
        '    If DT.Rows.Count > 0 Then
        ...
        For Each row As Windows.Forms.DataGridViewRow In GRIDBOOKINGS.Rows
            If row.Cells(1).Value <> Nothing And Convert.ToBoolean(row.Cells(GPASSCHK.Index).Value) = True Then
                ...
                If TXTBILLNO.Text.Trim <> "" And edit = False Then
                    Dim OBJCMN As New ClsCommon
                    Dim DT As DataTable = OBJCMN.search(" AIRLINEBOOKINGMASTER_DESC.BOOKING_CANCELLED AS CANCELLED", "", " AIRLINEBOOKINGMASTER INNER JOIN AIRLINEBOOKINGMASTER_DESC ON AIRLINEBOOKINGMASTER.BOOKING_NO = AIRLINEBOOKINGMASTER_DESC.BOOKING_NO AND AIRLINEBOOKINGMASTER.BOOKING_SALEREGISTERID = AIRLINEBOOKINGMASTER_DESC.BOOKING_SALEREGID AND AIRLINEBOOKINGMASTER.BOOKING_CMPID = AIRLINEBOOKINGMASTER_DESC.BOOKING_CMPID AND AIRLINEBOOKINGMASTER.BOOKING_LOCATIONID =AIRLINEBOOKINGMASTER_DESC.BOOKING_LOCATIONID AND AIRLINEBOOKINGMASTER.BOOKING_YEARID = AIRLINEBOOKINGMASTER_DESC.BOOKING_YEARID ", " AND AIRLINEBOOKINGMASTER.BOOKING_SALEBILLINITIALS = '" & TXTBILLNO.Text.Trim & "'  AND AIRLINEBOOKINGMASTER_DESC.BOOKING_GRIDSRNO = " & Val(row.Cells(GBOOKSRNO.Index).Value) & " AND AIRLINEBOOKINGMASTER.BOOKING_CMPID = " & CmpId & " AND AIRLINEBOOKINGMASTER.BOOKING_LOCATIONID = " & Locationid & " AND AIRLINEBOOKINGMASTER.BOOKING_YEARID = " & YearId)
                    If DT.Rows.Count > 0 Then
                        If Convert.ToBoolean(DT.Rows(0).Item("CANCELLED")) = True Then
                            EP.SetError(TXTBILLNO, "Credit Note Already Raised for Ticket No " & row.Cells(GTICKETNO.Index).Value)
                            bln = False
                        End If
                    End If
                End If
        ...
        If proceed = False Then
            ...
            'MessageBox.Show("Please Select At Least One Passenger to Continue.")
            bln = False
        End If
    ...
    Private Sub OpenToolStripButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles OpenToolStripButton.Click
        Try
            If USEREDIT = False And USERVIEW = False Then
                ...
                Exit Sub
            End If
            Dim OBJAIRCN As New AirlineCreditNoteDetails
            OBJAIRCN.MdiParent = MDIMain
            OBJAIRCN.Show()
            OBJAIRCN.BringToFront()
            Me.Close()
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Private Sub AirlineCreditNote_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
            ...
            ElseIf e.Alt = True And e.KeyCode = Windows.Forms.Keys.F1 Then
                Dim OBJINVDTLS As New AirlineCreditNoteDetails
    ...
    Private Sub AirlineCreditNote_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If BILLNO <> "" And edit = False Then
                ...
                GETDATA(" AND AIRLINEBOOKINGMASTER.BOOKING_SALEBILLINITIALS = '" & TXTBILLNO.Text.Trim & "' ")
            End If
            ...
            If edit = True Then
                ...
                Dim dt As New DataTable
                Dim objclsCN As New ClsAirCreditNote()
                ...
                If dt.Rows.Count > 0 Then
                    For Each dr As DataRow In dt.Rows
                        ...
                        TXTTOTALSALEAMT.Text = Format(Val(dt.Rows(0).Item("SALEAMT")), "0.00")
                        TXTEXTRACHGS.Text = Format(Val(dt.Rows(0).Item("EXTRACHGS")), "0.00")
                        TXTDISCPER.Text = Format(Val(dt.Rows(0).Item("DISCPER")), "0.00")
                        TXTDISCRS.Text = Format(Val(dt.Rows(0).Item("DISCRS")), "0.00")
                        TXTTDSPER.Text = Format(Val(dt.Rows(0).Item("TDSPER")), "0.00")
                        TXTTDSRS.Text = Format(Val(dt.Rows(0).Item("TDSRS")), "0.00")
                        ...
                        TXTCANCELINC.Text = Format(Val(dt.Rows(0).Item("CANCELINC")), "0.00")
...
LINE1:
            ...
            If Val(TXTCNNO.Text) - 1 >= TEMPCNNO Then
                edit = True
                AirlineCreditNote_Load(sender, e)
            Else
                clear()
                edit = False
            End If
            If GRIDBOOKINGS.RowCount = 0 And TEMPCNNO < MAXNO Then
                TXTCNNO.Text = TEMPCNNO
                GoTo LINE1
            End If
        Catch ex As Exception
...
LINE1:
            ...
            Else
                ...
                edit = False
            End If
            If GRIDBOOKINGS.RowCount = 0 And TEMPCNNO > 1 Then
                TXTCNNO.Text = TEMPCNNO
                GoTo LINE1
            End If
        Catch ex As Exception
        ...
        Try
            If edit = True Then
                ...
                If tempmsg = vbYes Then
                    ...
                    Dim OBJCN As New ClsAirCreditNote
                    Dim ALPARAVAL As New ArrayList
    ...
    Sub GETDATA(Optional ByVal WHERECLAUSE As String = "")
        Try
            ...
            Dim OBJCMN As New ClsCommon
            'Dim DT As DataTable = OBJCMN.search(" 'AIRLINEBOOKING' AS TYPE, AIRLINEBOOKINGMASTER.BOOKING_SALEBILLINITIALS AS BILL, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_PRINTNAME, '') AS CUSTOMER, FLIGHTMASTER.FLIGHT_NAME AS AIRLINE, LEDGERS.Acc_cmpname AS NAME, ISNULL(PURLEDGER.Acc_cmpname, '') AS PURNAME, BOOKEDBYMASTER.BOOKEDBY_NAME AS BOOKEDBY, AIRLINEBOOKINGMASTER.BOOKING_TICKETDATE AS TICKETDATE, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_PNRNO, '') AS PNR, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_AIRLINEPNR, '') AS AIRPNR, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_CRSPNR, '') AS CRSPNR, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_PARTYREFNO, '') AS PARTYREFNO, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_REFNO, '') AS REFNO, ISNULL(CRSMASTER.CRS_NAME, '') AS CRSTYPE, AIRLINEBOOKINGMASTER_DESC.BOOKING_GRIDSRNO AS GRIDSRNO, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_PASSNAME, '') AS PASSNAME, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_SECTOR, '') AS SECTOR, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_FLTNO, '') AS FLTNO, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_TYPE, '') AS BTYPE, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_TICKETNO, '') AS TICKETNO, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_CLASS, '') AS CLASS, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_BASIC, 0) AS BASIC, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_PSF, 0) AS PSF, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_TAXES, 0) AS TAXES, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_AMT, 0) AS AMT, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALEDISCPERC, 0) AS DISCPER, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALEDISCAMT, 0) AS DISCAMT, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALEADDDISCAMT, 0) AS ADDDISCAMT, ISNULL(OTHERCHGSLEDGER.Acc_cmpname, '') AS OTHERCHGSNAME, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_OTHERCHGS, 0) AS OTHERCHGS, ISNULL(TAXMASTER.tax_name, '') AS TAXNAME, ISNULL(TAXMASTER.tax_tax, 0) AS TAX, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALEAMTREC + AIRLINEBOOKINGMASTER.BOOKING_SALEEXTRAAMT,0) AS RECDAMT, isnull(AIRLINEBOOKINGMASTER.BOOKING_TOTALPASS,0) AS PAX, ISNULL(BOOKING_TAXSERVCHGS,0) AS TAXSERVCHGS, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_EXTRACHGS, '') AS EXTRACHGS, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALETDSPER, '') AS TDSPER, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALETDSRS, '') AS TDSRS ", "", " AIRLINEBOOKINGMASTER INNER JOIN AIRLINEBOOKINGMASTER_DESC ON AIRLINEBOOKINGMASTER.BOOKING_NO = AIRLINEBOOKINGMASTER_DESC.BOOKING_NO AND AIRLINEBOOKINGMASTER.BOOKING_SALEREGISTERID = AIRLINEBOOKINGMASTER_DESC.BOOKING_SALEREGID AND AIRLINEBOOKINGMASTER.BOOKING_YEARID = AIRLINEBOOKINGMASTER_DESC.BOOKING_YEARID INNER JOIN FLIGHTMASTER ON AIRLINEBOOKINGMASTER.BOOKING_AIRLINEID = FLIGHTMASTER.FLIGHT_ID LEFT OUTER JOIN LEDGERS AS OTHERCHGSLEDGER ON AIRLINEBOOKINGMASTER.BOOKING_OTHERCHGSID = OTHERCHGSLEDGER.Acc_id LEFT OUTER JOIN TAXMASTER ON AIRLINEBOOKINGMASTER.BOOKING_TAXID = TAXMASTER.tax_id LEFT OUTER JOIN BOOKEDBYMASTER ON AIRLINEBOOKINGMASTER.BOOKING_BOOKEDBYID = BOOKEDBYMASTER.BOOKEDBY_ID LEFT OUTER JOIN LEDGERS ON AIRLINEBOOKINGMASTER.BOOKING_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS PURLEDGER ON AIRLINEBOOKINGMASTER.BOOKING_PURLEDGERID = PURLEDGER.Acc_id LEFT OUTER JOIN CRSMASTER ON AIRLINEBOOKINGMASTER.BOOKING_CRSTYPEID = CRSMASTER.CRS_ID", WHERECLAUSE & " AND AIRLINEBOOKINGMASTER_DESC.BOOKING_CANCELLED = 'FALSE' AND AIRLINEBOOKINGMASTER.BOOKING_YEARID = " & YearId)

            Dim DT As DataTable = OBJCMN.search(" 'AIRLINEBOOKING' AS TYPE, AIRLINEBOOKINGMASTER.BOOKING_SALEBILLINITIALS AS BILL, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_PRINTNAME, '') AS CUSTOMER, FLIGHTMASTER.FLIGHT_NAME AS AIRLINE, LEDGERS.Acc_cmpname AS NAME, ISNULL(PURLEDGER.Acc_cmpname, '') AS PURNAME, BOOKEDBYMASTER.BOOKEDBY_NAME AS BOOKEDBY, AIRLINEBOOKINGMASTER.BOOKING_TICKETDATE AS TICKETDATE, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_PNRNO, '') AS PNR, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_AIRLINEPNR, '') AS AIRPNR, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_CRSPNR, '') AS CRSPNR, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_PARTYREFNO, '') AS PARTYREFNO, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_REFNO, '') AS REFNO, ISNULL(CRSMASTER.CRS_NAME, '') AS CRSTYPE, AIRLINEBOOKINGMASTER_DESC.BOOKING_GRIDSRNO AS GRIDSRNO, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_PASSNAME, '') AS PASSNAME, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_SECTOR, '') AS SECTOR, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_FLTNO, '') AS FLTNO, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_TYPE, '') AS BTYPE, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_TICKETNO, '') AS TICKETNO, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_CLASS, '') AS CLASS, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_BASIC, 0) AS BASIC, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_PSF, 0) AS PSF, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_TAXES, 0) AS TAXES, ISNULL(AIRLINEBOOKINGMASTER_DESC.BOOKING_AMT, 0) AS AMT, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALEDISCPERC, 0) AS DISCPER, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALEDISCAMT, 0) AS DISCAMT, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALEADDDISCAMT, 0) AS ADDDISCAMT, ISNULL(OTHERCHGSLEDGER.Acc_cmpname, '') AS OTHERCHGSNAME, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_OTHERCHGS, 0) AS OTHERCHGS, ISNULL(TAXMASTER.tax_name, '') AS TAXNAME, ISNULL(TAXMASTER.tax_tax, 0) AS TAX, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALEAMTREC + AIRLINEBOOKINGMASTER.BOOKING_SALEEXTRAAMT, 0) AS RECDAMT, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_TOTALPASS, 0) AS PAX, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_TAXSERVCHGS, 0) AS TAXSERVCHGS, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_EXTRACHGS, '') AS EXTRACHGS, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALETDSPER, '') AS TDSPER, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SALETDSRS, '') AS TDSRS, ISNULL(HSNMASTER.HSN_CODE, '') AS HSNCODE, ISNULL(HSNMASTER.HSN_ITEMDESC, '') AS ITEMDESC, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_CGSTPER, 0) AS CGSTPER, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_CGSTAMT, 0) AS CGSTAMT, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SGSTPER, 0) AS SGSTPER, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_SGSTAMT, 0) AS SGSTAMT, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_IGSTPER, 0) AS IGSTPER, ISNULL(AIRLINEBOOKINGMASTER.BOOKING_IGSTAMT, 0) AS IGSTAMT, ISNULL(CAST(STATEMASTER.state_remark AS VARCHAR), '') AS STATECODE ", "", "STATEMASTER RIGHT OUTER JOIN LEDGERS ON STATEMASTER.state_id = LEDGERS.Acc_stateid RIGHT OUTER JOIN AIRLINEBOOKINGMASTER INNER JOIN AIRLINEBOOKINGMASTER_DESC ON AIRLINEBOOKINGMASTER.BOOKING_NO = AIRLINEBOOKINGMASTER_DESC.BOOKING_NO AND AIRLINEBOOKINGMASTER.BOOKING_SALEREGISTERID = AIRLINEBOOKINGMASTER_DESC.BOOKING_SALEREGID AND AIRLINEBOOKINGMASTER.BOOKING_YEARID = AIRLINEBOOKINGMASTER_DESC.BOOKING_YEARID INNER JOIN FLIGHTMASTER ON AIRLINEBOOKINGMASTER.BOOKING_AIRLINEID = FLIGHTMASTER.FLIGHT_ID LEFT OUTER JOIN HSNMASTER ON AIRLINEBOOKINGMASTER.BOOKING_HSNCODEID = HSNMASTER.HSN_ID LEFT OUTER JOIN LEDGERS AS OTHERCHGSLEDGER ON AIRLINEBOOKINGMASTER.BOOKING_OTHERCHGSID = OTHERCHGSLEDGER.Acc_id LEFT OUTER JOIN TAXMASTER ON AIRLINEBOOKINGMASTER.BOOKING_TAXID = TAXMASTER.tax_id LEFT OUTER JOIN BOOKEDBYMASTER ON AIRLINEBOOKINGMASTER.BOOKING_BOOKEDBYID = BOOKEDBYMASTER.BOOKEDBY_ID ON LEDGERS.Acc_id = AIRLINEBOOKINGMASTER.BOOKING_LEDGERID LEFT OUTER JOIN LEDGERS AS PURLEDGER ON AIRLINEBOOKINGMASTER.BOOKING_PURLEDGERID = PURLEDGER.Acc_id LEFT OUTER JOIN CRSMASTER ON AIRLINEBOOKINGMASTER.BOOKING_CRSTYPEID = CRSMASTER.CRS_ID", WHERECLAUSE & " AND AIRLINEBOOKINGMASTER_DESC.BOOKING_CANCELLED = 'FALSE' AND AIRLINEBOOKINGMASTER.BOOKING_YEARID = " & YearId)
            If DT.Rows.Count > 0 Then
                For Each DTROW As DataRow In DT.Rows

                    If DTROW("RECDAMT") > 0 Then
                    ...
                    TXTEXTRACHGS.Text = DTROW("EXTRACHGS")
                    TXTDISCPER.Text = DTROW("DISCPER")
                    TXTDISCRS.Text = DTROW("DISCAMT")
                    TXTTDSPER.Text = DTROW("TDSPER")
                    TXTTDSRS.Text = DTROW("TDSRS")
    ...
    Sub getsrno(ByRef grid As System.Windows.Forms.DataGridView)
        Try
            For Each row As DataGridViewRow In grid.Rows
                If Convert.ToBoolean(row.Cells(0).Value) = True Then row.Cells(1).Value = row.Index + 1
            Next
        Catch ex As Exception
            If ErrHandle(ex.Message.GetHashCode) = False Then Throw ex
        End Try
    ...
    Private Sub TXTADDDISC_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles TXTADDDISC.Validated
        TOTAL()
    End Sub
    ...
    Private Sub AirlineCreditNote_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
    ...
    Sub PRINTREPORT(ByVal BILLNO As Integer)
        Try
            ...
            If TEMPMSG = vbYes Then
                ...
                OBJCN.FRMSTRING = "AIRCREDIT"
                OBJCN.printot = CHKOTHERCHGSPRINT.Checked
                OBJCN.printst = CHKSTPRINT.Checked
                OBJCN.Show()
            End If
        Catch ex As Exception
            Throw ex
    ...
    Private Sub TXTCANCEL_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles TXTCANCEL.Validated
        If Val(TXTCANCEL.Text.Trim) > 0 Then
            INCOMECCGROUP.Visible = True
            TXTCANCELINC.Focus()
            TOTAL()
        End If
    End Sub

    Private Sub TXTCANCELINC_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TXTCANCELINC.KeyPress
        numdotkeypress(e, TXTCANCELINC, Me)
    End Sub
        ...
        Try
            If edit = True And TXTIRNNO.Text.Trim <> "" And IsNothing(PBQRCODE.Image) = True Then
                ...
                If TEMPSTATUS = "FAILED" Then
                    ...
                    DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTCNNO.Text.Trim) & ",'AIRCREDITNOTE','" & TOKEN & "','','" & TEMPSTATUS & "','" & Replace(REQUESTEDTEXT, "'", "''") & "', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                    Exit Sub
                End If
                ...
                Dim TEMPREG As DataTable = OBJCMN.search(" REGISTER_INITIALS AS INITIALS", "", " REGISTERMASTER ", " AND REGISTER_NAME = '" & cmbregister.Text.Trim & "' and register_type = 'AIRLINE CREDITNOTE' AND REGISTER_YEARID = " & YearId)
                ...
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTCNNO.Text.Trim) & ",'AIRCREDITNOTE','" & TOKEN & "','" & TXTIRNNO.Text.Trim & "','QRCODE SUCCESS', '', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTCNNO.Text.Trim) & ",'AIRCREDITNOTE','" & TOKEN & "','" & TXTIRNNO.Text.Trim & "','QRCODE SUCCESS1', '', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
    ...
    Private Sub CHKTAXSERVCHGS_CheckedChanged(sender As Object, e As EventArgs) Handles CHKTAXSERVCHGS.CheckedChanged
        TOTAL()
    End Sub
    ...
    Async Sub GENERATEINV()
        Try
            ...
            'PARTY GST DETAILS 
            DT = OBJCMN.search(" ISNULL(ACC_GSTIN, '') AS GSTIN, ISNULL(ACC_ZIPCODE,'') AS PINCODE, ISNULL(STATE_NAME,'') AS STATENAME, ISNULL(CAST(STATE_REMARK AS VARCHAR(20)),'') AS STATECODE, ISNULL(ACC_ADD1,'') AS ADD1, ISNULL(ACC_ADD2,'') AS ADD2, GROUPMASTER.GROUP_SECONDARY AS SECONDARY  ", "", " LEDGERS LEFT OUTER JOIN STATEMASTER ON ACC_STATEID = STATE_ID INNER JOIN GROUPMASTER ON LEDGERS.ACC_GROUPID = GROUPMASTER.GROUP_ID", " AND ACC_CMPNAME = '" & TXTNAME.Text.Trim & "' AND ACC_YEARID = " & YearId)
            ...
            Try
                ...
                Dim DTINI As DataTable = OBJCMN.search("AIRCN_INITIALS AS PRINTINITIALS", "", " AIRCREDITNOTEMASTER ", " AND AIRCN_NO = " & Val(TXTCNNO.Text.Trim) & " AND AIRCN_YEARID = " & YearId)
                ...
                If CHKTAXSERVCHGS.Checked = True Then
                    TEMPOTHERAMT = Val(TXTTOTALSALEAMT.Text.Trim) - Val(TXTDISCRS.Text.Trim) + Val(txttax.Text.Trim) - Val(TXTADDDISC.Text.Trim) + Val(txtotherchg.Text.Trim) + Val(TXTCANCEL.Text.Trim)
                    TEMPTOTALITEMAMT = Val(TXTEXTRACHGS.Text.Trim) + Val(TXTCGSTAMT.Text.Trim) + Val(TXTSGSTAMT.Text.Trim) + Val(TXTIGSTAMT.Text.Trim)
                Else
                    TEMPOTHERAMT = Val(TXTTOTALPSF.Text.Trim) + Val(TXTTOTALTAXES.Text.Trim) + Val(TXTEXTRACHGS.Text.Trim) - Val(TXTDISCRS.Text.Trim) + Val(txttax.Text.Trim) - Val(TXTADDDISC.Text.Trim) + Val(txtotherchg.Text.Trim) + Val(TXTCANCEL.Text.Trim)
...
ERRORMESSAGE:
                ...
                'ADD DATA IN EINVOICEENTRY
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTCNNO.Text.Trim) & ",'AIRCREDITNOTE','" & TOKEN & "','','FAILED','" & Replace(REQUESTEDTEXT, "'", "''") & "', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
            ...
            DT = OBJCMN.Execute_Any_String("UPDATE AIRCREDITNOTEMASTER SET AIRCN_IRNNO = '" & TXTIRNNO.Text.Trim & "', AIRCN_ACKNO = '" & TXTACKNO.Text.Trim & "', AIRCN_ACKDATE = '" & Format(ACKDATE.Value.Date, "MM/dd/yyyy") & "' FROM AIRCREDITNOTEMASTER WHERE AIRCN_NO = " & Val(TXTCNNO.Text.Trim) & " AND AIRCN_YEARID = " & YearId, "", "")

            'ADD DATA IN EINVOICEENTRY
            DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTCNNO.Text.Trim) & ",'AIRCREDITNOTE','" & TOKEN & "','" & IRNNO & "','" & TEMPSTATUS & "', '', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
            ...
            If TEMPSTATUS = "SUCCESS" Then
                ...
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTCNNO.Text.Trim) & ",'AIRCREDITNOTE','" & TOKEN & "','" & IRNNO & "','QRCODE SUCCESS', '', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTCNNO.Text.Trim) & ",'AIRCREDITNOTE','" & TOKEN & "','" & IRNNO & "','QRCODE SUCCESS1', '', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
    ...
    Private Sub TXTCANCELINC_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles TXTCANCELINC.Validated
        INCOMECCGROUP.Visible = False
        txtremarks.Focus()
    End Sub
    ...
    Private Sub txttax_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txttax.Validated, TXTEXTRACHGS.Validated
        TOTAL()
    End Sub
