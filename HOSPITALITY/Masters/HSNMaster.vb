...
Public Class HSNMaster
    ...
    Public TEMPHSNNO, tempHSNCODE, temphsnitemdesc As String
    ...
    Private Function errorvalid() As Boolean
        ...
        If Val(TXTSGST.Text.Trim) = 0 Then
            EP.SetError(TXTDESC, "Enter SGST")
            bln = False
        End If

        If Val(TXTIGST.Text.Trim) = 0 Then
            EP.SetError(TXTDESC, "Enter IGST")
            bln = False
        End If
        ...
        Try
            ...
            If (EDIT = False) Or (EDIT = True And LCase(TXTHSNCODE.Text) <> LCase(tempHSNCODE)) Then
                ...
                If dt.Rows.Count > 0 Then
                    MsgBox("Code Already Exists", MsgBoxStyle.Critical, "HOSPITALITY")
                    BLN = False
                End If
            End If
            Return BLN
    ...
    Private Sub TXTITEMDESC_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTITEMDESC.Validating
        Try
            If TXTITEMDESC.Text <> "" Then
                If EDIT = False Or (EDIT = True And temphsnitemdesc <> TXTITEMDESC.Text.Trim) Then
                    Dim OBJCMN As New ClsCommon
                    Dim dttable As DataTable = OBJCMN.search(" HSN_ITEMDESC ", "", " HSNMASTER ", " and HSN_ITEMDESC = '" & TXTITEMDESC.Text.Trim & "'  And HSN_cmpid = " & CmpId & " And HSN_yearid = " & YearId)
                    If dttable.Rows.Count > 0 Then
                        MsgBox("Item Description Already Exist")
                        e.Cancel = True
                    End If
                End If
            End If
